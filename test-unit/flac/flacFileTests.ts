...
@suite
class Flac_File_ConstructorTests {
	@test
	...
	public constructor_hasTagsAtBeginning() {
		...
		try {
			...
			const fileBytes = ByteVector.concatenate(tagBytes, Flac_File_ConstructorTests.getBasicFile());
	...
	public constructor_hasTagsAtEnd() {
		...
		try {
			...
			const fileBytes = ByteVector.concatenate(Flac_File_ConstructorTests.getBasicFile(), tagBytes);
			const testAbstraction = TestFile.getFileAbstraction(fileBytes.toByteVector());
	...
	public constructor_hasAllTheTags() {
		...
		try {
			...
			const fileBytes = ByteVector.concatenate(startTagBytes, Flac_File_ConstructorTests.getBasicFile(extraBlocks), endTagBytes);
			const testAbstraction = TestFile.getFileAbstraction(fileBytes.toByteVector());
	...
	public constructor_averageRead_doesNotHaveStreamInfoBlock() {
		// Arrange
		const fileBytes = ByteVector.concatenate(ByteVector.fromString("fLaC", StringType.Latin1), ByteVector.fromSize(20, 0xff));
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public save_noTagsOriginally_addTagsAtStart() {
		...
		const expectedBytes = ByteVector.concatenate(startTagBytes, Flac_File_ConstructorTests.getBasicFile([paddingBlock]));
		Testers.bvEqual(testAbstraction.allBytes, expectedBytes);
	...
	public save_noTagsOriginally_addTagsAtEnd() {
		...
		const expectedBytes = ByteVector.concatenate(Flac_File_ConstructorTests.getBasicFile([paddingBlock]), endBytes);
		Testers.bvEqual(testAbstraction.allBytes, expectedBytes);
	...
	public save_hasTagsOriginally_newBlocksSmaller() {
		...
		// Assert
		const paddingLength =
			Flac_File_ConstructorTests.standardPadding +
			Flac_File_ConstructorTests.getXiphTag().render(false).length +
			FlacBlock.HEADER_SIZE +
		...
		const expectedBytes = ByteVector.concatenate(startTags, Flac_File_ConstructorTests.getBasicFile([paddingBlock]), endTags);
		Testers.bvEqual(testAbstraction.allBytes, expectedBytes);
	...
	private static getCompleteFile(): ByteVector {
		...
		const startTagBytes = ByteVector.concatenate(apeTag.render(), id3v2Tag.render());
		const endTagBytes = id3v1Tag.render();
		return ByteVector.concatenate(startTagBytes, Flac_File_ConstructorTests.getBasicFile(extraBlocks), endTagBytes);
	}
}
