...
class Flac_BlockTests {
	...
	public fromFile_validIsLastBlock() {
		// Arrange
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(10), ByteVector.fromUint(0x86123456), ByteVector.fromSize(10));
	...
	public fromFile_validIsNotLastBlock() {
		// Arrange
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(10), ByteVector.fromUint(0x06123456), ByteVector.fromSize(10));
		const file = TestFile.getFile(fileBytes);
	...
	public fromFile_loadsOnDataAccess() {
		// Arrange
		const fileBytes = ByteVector.concatenate(ByteVector.fromUint(0x0600000a), ByteVector.fromSize(10, 0x0b));
		const file = TestFile.getFile(fileBytes);
	...
	public render_isLastBlock() {
		...
		// Assert
		const expected = ByteVector.concatenate(0x81, 0x00, 0x00, 0x0a, data);
	...
	public render_isNotLastBlock() {
		...
		// Assert
		const expected = ByteVector.concatenate(0x01, 0x00, 0x00, 0x0a, data);
		Testers.bvEqual(output, expected);
	}
}
