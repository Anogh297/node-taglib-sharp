...
class StreamTests {
	...
	public read() {
		...
		try {
			...
			// Assert
			const expected1 = new Uint8Array([TestConstants.testFileContents[0], TestConstants.testFileContents[1], TestConstants.testFileContents[2], 0, 0]);
			...
			// Assert
			const expected2 = new Uint8Array([0, TestConstants.testFileContents[3], TestConstants.testFileContents[4], TestConstants.testFileContents[5], 0]);
	...
	public seek_current_invalidPosition() {
		...
		try {
			...
			Testers.testSafeInt((v: number) => {
				stream.seek(v, SeekOrigin.Current);
			});
	...
	public write_writeAtEnd() {
		const testAction = (testFilePath: string, stream: Stream) => {
			...
			const contents = fs.readFileSync(testFilePath);
			const expected = ByteVector.concatenate(ByteVector.fromString(testFilePath, StringType.Latin1), buffer);
			assert.deepStrictEqual(contents, expected["_bytes"]);
		};
		StreamTests.testWithFile(testAction, true);
	}
	...
	private static testWithFile(testAction: (filePath: string, stream: Stream) => void, isWritable: boolean) {
		...
		const stream = isWritable ? Stream.createAsReadWrite(testFilePath) : Stream.createAsRead(testFilePath);
