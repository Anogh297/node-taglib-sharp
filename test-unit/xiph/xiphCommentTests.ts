...
@suite
class Xiph_Comment_ConstructorTests {
	@test
	...
	public fromData_noData() {
		// Arrange
		const data = ByteVector.concatenate(ByteVector.fromUint(9, false), ByteVector.fromString("foobarbaz", StringType.UTF8), ByteVector.fromUint(0, false));
...
@suite
class Xiph_Comment_PropertyTests {
	@test
	public title() {
		this.testTextItem(
			(t, v) => {
	...
	private testTextArrayItem(set: (t: XiphComment, v: string[]) => void, get: (t: XiphComment) => string[], key: string) {
		// Arrange
		const tag = XiphComment.fromEmpty();
	...
	private testTextItem(set: (t: XiphComment, v: string) => void, get: (t: XiphComment) => string, key: string) {
		// Arrange
		const tag = XiphComment.fromEmpty();
		const setProp = (v: string) => set(tag, v);
		const getProp = () => get(tag);
	...
	private testUintItem(set: (t: XiphComment, v: number) => void, get: (t: XiphComment) => number, key: string) {
		// Arrange
		const tag = XiphComment.fromEmpty();
...
@suite
class Xiph_Comment_MethodTests {
	private static readonly vendorId = "foo";
	private static readonly title1 = "bar";
	private static readonly title2 = "baz";
	private static readonly encodedOldPictureData = ByteVector.fromString("fuxbuxqux", StringType.UTF8).toBase64String();
	...
	public render_emptyCommentNoFramingBit() {
		...
		// Assert
		const expectedOutput = ByteVector.concatenate(ByteVector.fromUint(0, false), ByteVector.fromUint(0, false));
	...
	public render_emptyCommentHasFramingBit() {
		...
		// Assert
		const expectedOutput = ByteVector.concatenate(ByteVector.fromUint(0, false), ByteVector.fromUint(0, false), 0x01);
		Testers.bvEqual(output, expectedOutput);
	}
