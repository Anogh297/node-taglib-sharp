...
class PictureLazy_Tests {
	...
	public fromData_canGetExtension() {
		// Arrange
		const data = ByteVector.concatenate(0xff, 0xd8, 0xff, ByteVector.fromString("foobarbaz", StringType.Latin1));
	...
	public fromFile_invalidParameters() {
		...
		Testers.testTruthy((v: IFileAbstraction) => {
			PictureLazy.fromFile(v, 0, 0);
		});
		Testers.testInt((v: number) => {
			PictureLazy.fromFile(mockFile.object, v, 0);
		});
		Testers.testInt((v: number) => {
			PictureLazy.fromFile(mockFile.object, v, 0);
		}, Allow.Undefined);
	...
	public fromFile_hasImageFilenameNoOffsetReadAll() {
		...
		// Assert
		PictureLazy_Tests.assertFileLazyPicture(picture, data, "foobarbaz.jpg", "foobarbaz.jpg", "image/jpeg", PictureType.FrontCover);
	}
	...
	public fromFile_hasNonImageFilenameNoOffsetReadAll() {
		...
		// Assert
		PictureLazy_Tests.assertFileLazyPicture(picture, data, "foobarbaz.bin", "foobarbaz.bin", "application/octet-stream", PictureType.NotAPicture);
	}
	...
	public fromFile_noExtensionNoOffsetReadAll() {
		...
		// Assert
		PictureLazy_Tests.assertFileLazyPicture(picture, data, "fuxbuxqux", "fuxbuxqux", "image/jpeg", PictureType.FrontCover);
	}
	...
	public fromFile_noExtensionNotImageNoOffsetReadAll() {
		...
		// Assert
		PictureLazy_Tests.assertFileLazyPicture(picture, data, "fuxbuxqux", "fuxbuxqux", "application/octet-stream", PictureType.NotAPicture);
	}
	...
	public fromFile_noFilenameNotImageFileNoOffsetReadAll() {
		...
		// Assert
		PictureLazy_Tests.assertFileLazyPicture(picture, data, undefined, "UnknownType", "application/octet-stream", PictureType.NotAPicture);
	}

	@test
	public fromFile_hasOffsetReadAll() {
		const data = ByteVector.fromString("foobarbaz", StringType.Latin1);
		const allData = ByteVector.concatenate(0x00, 0x00, data);
	...
	public fromFile_hasOffsetReadPartial() {
		const data = ByteVector.fromString("foobarbaz", StringType.Latin1);
		const allData = ByteVector.concatenate(0x00, 0x00, data, 0x00, 0x00);
		const mockStream = new TestStream(allData, false);
	...
	private static assertFileLazyPicture(picture: PictureLazy, d: ByteVector, desc: string, f: string, mt: string, t: PictureType) {
		assert.isOk(picture);
