...
export default {
	mockFile: (): File => {
		return TypeMoq.Mock.ofType<File>().object;
	},
	getFile: (data: ByteVector | number[]): File => {
		const mockFile = TypeMoq.Mock.ofType<File>();
		let position = 0;

		const dataBv = data instanceof ByteVector ? data : ByteVector.fromByteArray(data);

		mockFile.setup((f) => f.length).returns(() => dataBv.length);
		mockFile
			.setup((f) => f.seek(TypeMoq.It.isAnyNumber(), <SeekOrigin>TypeMoq.It.isAny()))
		...
		mockFile
			.setup((f) => f.readBlock(TypeMoq.It.isAnyNumber()))
			.returns((s: number) => {
	...
	getFileAbstraction: (data: ByteVector): TestFileAbstraction => {
		return {
			...
			get allBytes(): ByteVector {
				return data;
			},
			get readStream(): IStream {
				return new TestStream(data, false);
			},
			get writeStream(): IStream {
				return new TestStream(data, true);
			},
