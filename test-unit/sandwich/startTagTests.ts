...
class Sandwich_StartTagTests {
	...
	public constructor_tagsAtEnd() {
		...
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), testTag.render());
		const file = TestFile.getFile(fileBytes);
	...
	public createTag_copy() {
		...
		const fileBytes = ByteVector.concatenate(id3v2Tag.render(), ByteVector.fromSize(100));
		const file = TestFile.getFile(fileBytes);
	...
	public render_hasTags() {
		...
		// Assert
		const expectedBytes = ByteVector.concatenate(id3v2Tag.render(), apeTag.render());
