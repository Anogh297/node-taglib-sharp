...
@suite
class Sandwich_FileTests {
	private readonly testMapping = new Map<TagTypes, () => boolean>();

	@test
	public constructor_invalidParams() {
		// Act / Assert
		Testers.testTruthy<string | IFileAbstraction>((v) => new TestSandwichFile(v, ReadStyle.None, this.testMapping, TagTypes.None));
	}
	...
	public constructor_hasTags_noDefaults() {
		...
		apeTag.amazonId = "bar";
		const fileBytes = ByteVector.concatenate(id3v2Tag.render(), apeTag.render(), ByteVector.fromSize(100), id3v1Tag.render());
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public save_noTags_addTags() {
		...
		// Assert
		const expectedBytes = ByteVector.concatenate((<ApeTag>startTag).render(), ByteVector.fromSize(100), (<Id3v2Tag>endTag).render());
	...
	public save_hasTags_addTags() {
		...
		id3v2Tag.album = "foo";
		const fileBytes = ByteVector.concatenate(id3v2Tag.render(), ByteVector.fromSize(100), id3v1Tag.render());
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
		...
		// Assert
		const expectedBytes = ByteVector.concatenate(id3v2Tag.render(), (<ApeTag>newTag).render(), ByteVector.fromSize(100), id3v1Tag.render());
		Testers.bvEqual(testAbstraction.allBytes, expectedBytes);
		assert.strictEqual(file.mode, FileAccessMode.Closed);
	...
	public save_hasTags_removeTags() {
		...
		// Assert
		const expectedBytes = ByteVector.concatenate(id3v2Tag.render(), ByteVector.fromSize(100), id3v1Tag.render());
		Testers.bvEqual(testAbstraction.allBytes, expectedBytes);
		assert.strictEqual(file.mode, FileAccessMode.Closed);
...
class TestSandwichFile extends SandwichFile {
	...
	public constructor(fileToRead: IFileAbstraction | string, readStyle: ReadStyle, defaultTagMappingTable: Map<TagTypes, () => boolean>, defaultTags: TagTypes) {
