...
class Sandwich_TagTests {
	...
	public constructor_tagsAtStart() {
		...
		apeTag.amazonId = "bar";
		const fileBytes = ByteVector.concatenate(id3v2Tag.render(), apeTag.render(), ByteVector.fromSize(100));
	...
	public constructor_tagsAtEnd() {
		...
		const apeTag = ApeTag.fromEmpty();
		apeTag.amazonId = "bar";
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), id3v2Tag.render(), apeTag.render());
		const file = TestFile.getFile(fileBytes);
		const testMapping = new Map<TagTypes, () => boolean>();
	...
	public createTag_alreadyExistsOnOppositeSideOfFile() {
		...
		const file = TestFile.getFile(fileBytes);
		const testMapping = new Map<TagTypes, () => boolean>([[TagTypes.Ape, () => true]]);
		const tag = new SandwichTag(file, ReadStyle.None, testMapping);
