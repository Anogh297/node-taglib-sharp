...
class Sandwich_EndTagTests {
	...
	public constructor_tagsAtStart() {
		...
		const fileBytes = ByteVector.concatenate(testTag.render(), ByteVector.fromSize(100));
		const file = TestFile.getFile(fileBytes);
	...
	public constructor_tagsAtEnd_tagsSmallerThanReadSize() {
		...
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), id3v2Tag.render(), apeTag.render(), id3v1Tag.render());
		const file = TestFile.getFile(fileBytes);
	...
	public constructor_tagsAtEnd_tagsLargerThanReadSize() {
		...
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), apeTag.render(), id3v2Tag.render(), id3v1Tag.render());
		const file = TestFile.getFile(fileBytes);
	...
	public createTag_copy() {
		...
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), id3v2Tag.render());
		const file = TestFile.getFile(fileBytes);
	...
	public render_hasTags() {
		...
		const fileBytes = ByteVector.concatenate(ByteVector.fromSize(100), id3v2Tag.render(), id3v1Tag.render(), apeTag.render());
		const file = TestFile.getFile(fileBytes);
		const tag = new EndTag(file, ReadStyle.None);
		...
		// Assert
		const expectedBytes = ByteVector.concatenate(apeTag.render(), id3v2Tag.render(), id3v1Tag.render());
