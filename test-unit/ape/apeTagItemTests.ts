...
@suite
class Ape_TagItemTest_ConstructorTests {
	@test
	public fromBinaryValue_invalidValues() {
		...
		Testers.testTruthy((v: string) => {
			ApeTagItem.fromBinaryValue(v, bv);
		});
		Testers.testTruthy((v: ByteVector) => {
			ApeTagItem.fromBinaryValue("", v);
		});
	}
	...
	public fromData_invalidValues() {
		...
		// Act / Assert
		Testers.testTruthy((v: ByteVector) => {
			ApeTagItem.fromData(v, 0);
		});
		Testers.testUint((v: number) => {
			ApeTagItem.fromData(data, v);
		});
	}
...
@suite
class Ape_TagItemTest_MethodTests {
	@test
	public clone_binary() {
		// Arrange
		const key = "key";
