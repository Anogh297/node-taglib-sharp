...
@suite
class Ape_Tag_ConstructorTests {
	@test
	...
	public fromData_tagWithItems() {
		...
		const data = ByteVector.concatenate(item1, item2, getTestTagFooter(0, 3, item1.length + item2.length + ApeTagFooter.SIZE));
	...
	public fromFile_invalidParams() {
		...
		Testers.testUint((v: number) => {
			ApeTag.fromFile(mockFile.object, v);
		});
		assert.throws(() => {
			ApeTag.fromFile(mockFile.object, 5);
		});
	}
	...
	public fromFile_emptyTag() {
		// Arrange
		const data = ByteVector.concatenate(ByteVector.fromSize(10), getTestTagFooter(ApeTagFooterFlags.HeaderPresent, 0, ApeTagFooter.SIZE));
	...
	public fromFile_positionIsAtFooter() {
		...
		const item1 = ApeTagItem.fromTextValues("key1", "value1").render();
		const item2 = ApeTagItem.fromTextValues("key2", "value2").render();
		const data = ByteVector.concatenate(ByteVector.fromSize(10), item1, item2, getTestTagFooter(0, 3, item1.length + item2.length + ApeTagFooter.SIZE));
	...
	public fromFile_positionIsAtHeader() {
		...
		const data = ByteVector.concatenate(ByteVector.fromSize(10), header, footer);
		const file = TestFile.getFile(data);
...
class Ape_Tag_PropertyTests {
	...
	@test
	public initialKey() {
		Ape_Tag_PropertyTests.testUnsupportedText(
			(t, v) => {
				t.initialKey = v;
			},
			(t) => t.initialKey,
	...
	private testTextArrayItem(set: (t: ApeTag, v: string[]) => void, get: (t: ApeTag) => string[], key: string) {
		// Arrange
		const tag = ApeTag.fromEmpty();
	...
	private testTextItem(set: (t: ApeTag, v: string) => void, get: (t: ApeTag) => string, key: string) {
		// Arrange
		const tag = ApeTag.fromEmpty();
		const setProp = (v: string) => set(tag, v);
		const getProp = () => get(tag);
	...
	private testUintItem(set: (t: ApeTag, v: number) => void, get: (t: ApeTag) => number, key: string) {
		// Arrange
		const tag = ApeTag.fromEmpty();
	...
	private static testUnsupportedText(set: (t: ApeTag, v: string) => void, get: (t: ApeTag) => string) {
		// Arrange
		const tag = ApeTag.fromEmpty();
...
class Ape_Tag_MethodTests {
	...
	public appendStringValue_invalidValues() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.appendStringValue(undefined, "abc");
		});
		assert.throws(() => {
			tag.appendStringValue(null, "abc");
		});
	}
	...
	public appendStringValues_invalidValues() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.appendStringValues(undefined, ["abc"]);
		});
		assert.throws(() => {
			tag.appendStringValues(null, ["abc"]);
		});
	}
	...
	public copyTo_invalidDestination() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.copyTo(undefined, true);
		});
		assert.throws(() => {
			tag.copyTo(null, true);
		});
	}
	...
	public render() {
		...
		const expected = ByteVector.concatenate(header, item1Render, item2Render, footer);
	...
	public setNumericValue_invalidValues() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.setNumericValue(undefined, 0, 0);
		});
		assert.throws(() => {
			tag.setNumericValue(null, 0, 0);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", -1, 0);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", 1.23, 0);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", Number.MAX_SAFE_INTEGER + 1, 0);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", 0, -1);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", 0, 1.23);
		});
		assert.throws(() => {
			tag.setNumericValue("foo", 0, Number.MAX_SAFE_INTEGER + 1);
		});
	}
	...
	public setStringValue_invalidValue() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.setStringValue(undefined, "foo");
		});
		assert.throws(() => {
			tag.setStringValue(null, "foo");
		});
	}
	...
	public setStringValues_invalidValue() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.setStringValues(undefined, ["foo"]);
		});
		assert.throws(() => {
			tag.setStringValues(null, ["foo"]);
		});
	}
