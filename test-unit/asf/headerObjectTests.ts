...
@suite
class Asf_HeaderObjectTests extends ObjectTests<HeaderObject> {
	protected get fromFileConstructor(): (f: File, p: number) => HeaderObject {
		return HeaderObject.fromFile;
	}
	protected get minSize(): number {
		return 26;
	}
	protected get objectGuid(): UuidWrapper {
		return Guids.ASF_HEADER_OBJECT;
	}
	...
	public removeContentDescriptor_withMatches() {
		...
		const headerBytes = Asf_HeaderObjectTests.getObjectBytesFromObjects([contentDescriptionObject, extendedContentDescription]);
		const headerFile = TestFile.getFile(headerBytes);
		const headerObject = HeaderObject.fromFile(headerFile, 10);
	...
	public render_objectGrew_adds4096Padding() {
		...
		// Assert
		const expectedChildren = [unknownObject, newObject, PaddingObject.fromSize(4096)];
