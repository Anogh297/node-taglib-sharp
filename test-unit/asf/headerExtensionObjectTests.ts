...
@suite
class Asf_HeaderExtensionObjectTests extends ObjectTests<HeaderExtensionObject> {
	...
	protected get objectGuid(): UuidWrapper {
		return Guids.ASF_HEADER_EXTENSION_OBJECT;
	}
	...
	public fromFile_missingReservedField1() {
		...
		// Act / Assert
		assert.throws(() => {
			HeaderExtensionObject.fromFile(file, 10);
		});
	}
	...
	public render_isRoundTrip() {
		// Arrange
		const data = Asf_HeaderExtensionObjectTests.getObjectBytes([MetadataLibraryObject.fromEmpty(), ContentDescriptionObject.fromEmpty()]);
		const file = TestFile.getFile(data);
