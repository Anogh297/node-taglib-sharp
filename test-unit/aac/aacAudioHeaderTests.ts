...
class Aac_AudioHeaderTests {
	...
	public find_invalidParameters() {
		...
		Testers.testTruthy((v: File) => {
			AacAudioHeader.find(v, 0, 0);
		});
		Testers.testUint((v: number) => {
			AacAudioHeader.find(mockFile.object, v, 0);
		});
		Testers.testUint((v: number) => {
			AacAudioHeader.find(mockFile.object, 0, v);
		}, Allow.Undefined);
	}
	...
	public find_headerNotWithinByteLimit() {
		// Arrange
		const data = ByteVector.concatenate(ByteVector.fromSize(22), sampleHeaderBytes);
		const mockFile = TestFile.getFile(data);
	...
	public find_headerInFileRequiresMultipleBufferReads() {
		// Arrange
		const data = ByteVector.concatenate(ByteVector.fromSize(File.bufferSize * 2.5 + 1), sampleHeaderBytes, ByteVector.fromSize(20));
		const mockFile = TestFile.getFile(data);
