...
class Riff_AviHeaderTests {
	...
	public constructor_hasUnsupportedStreams() {
		...
		const header1Data = ByteVector.concatenate(ByteVector.fromUint(AviStreamType.MidiStream, false), ByteVector.fromSize(52));
		...
		const header2Data = ByteVector.concatenate(ByteVector.fromUint(AviStreamType.TextStream, false), ByteVector.fromSize(52));
	...
	public constructor_hasSupportedStreams() {
		...
		const header1Data = ByteVector.concatenate(ByteVector.fromUint(AviStreamType.AudioStream, false), ByteVector.fromSize(52));
		const streamList1 = RiffList.fromEmpty(AviStream.LIST_TYPE);
		streamList1.setValues(AviStream.HEADER_CHUNK_ID, [header1Data]);
