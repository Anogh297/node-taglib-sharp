...
class Riff_RiffFileTests {
	...
	public constructor_nonRiffFile() {
		// Arrange
		const fileBytes = ByteVector.concatenate(ByteVector.fromString("FOOO", StringType.UTF8), ByteVector.fromUint(100, false), Resources.getDataChunk());
	...
	public constructor_aviFile_noTagsNoCodecsAllDefaultsCreated() {
		...
		try {
			const dataBytes = ByteVector.concatenate(ByteVector.fromString("AVI ", StringType.UTF8), Resources.getAviHeaderBlock(false), Resources.getMoviChunk());
	...
	public constructor_aviFile_noTagsHasCodecsReadAllDefaultsCreated() {
		...
		const originalDefaults = AviFileSettings.defaultTagTypes;
		try {
			const dataBytes = ByteVector.concatenate(ByteVector.fromString("AVI ", StringType.UTF8), Resources.getAviHeaderBlock(true), Resources.getMoviChunk());
			const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
			const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public constructor_aviFile_missingHeaders() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("AVI ", StringType.UTF8), Resources.getMoviChunk());
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public constructor_unsupportedType() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("FOOO", StringType.UTF8), Resources.getMoviChunk());
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public constructor_waveFile_noTagsCodecsAreReadAllDefaultsCreated() {
		...
		try {
			const dataBytes = ByteVector.concatenate(ByteVector.fromString("WAVE", StringType.UTF8), Resources.getWaveFormatBlock(), Resources.getDataChunk());
			const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
			const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public constructor_waveFile_missingHeaders() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("WAVE", StringType.UTF8), Resources.getDataChunk());
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public constructor_waveFile_noDataBlock() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("WAVE", StringType.UTF8), Resources.getWaveFormatBlock());
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public save_waveNoTagsNoDataChunk_addingTags() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("WAVE", StringType.UTF8), Resources.getWaveFormatBlock(), Resources.getJunkChunk(10));
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	public save_aviNoTagsNoMoviChunk_addingTags() {
		// Arrange
		const dataBytes = ByteVector.concatenate(ByteVector.fromString("AVI ", StringType.UTF8), Resources.getAviHeaderBlock(true), Resources.getJunkChunk(10));
		const fileBytes = Riff_RiffFileTests.getFileBytes(dataBytes);
		const testAbstraction = TestFile.getFileAbstraction(fileBytes);
	...
	private static getFileBytes(dataBytes: ByteVector): ByteVector {
		return ByteVector.concatenate(RiffFile.FILE_IDENTIFIER, ByteVector.fromUint(dataBytes.length, false), dataBytes);
	}
