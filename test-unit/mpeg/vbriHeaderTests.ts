...
class Mpeg_XingHeaderTests {
	...
	public fromInfo_invalidParameters() {
		...
		Testers.testUint((v: number) => {
			VbriHeader.fromInfo(v, 123);
		});
		Testers.testUint((v: number) => {
			VbriHeader.fromInfo(123, v);
		});
	}
	...
	public fromData_invalidParameters() {
		// Act / Assert
		assert.throws(() => {
			VbriHeader.fromData(undefined);
		});
		assert.throws(() => {
			VbriHeader.fromData(null);
		});
	}
	...
	public fromData_validParameters() {
		// Arrange
		const data = ByteVector.concatenate(VbriHeader.FILE_IDENTIFIER, ByteVector.fromSize(6), ByteVector.fromUint(123), ByteVector.fromUint(234));
