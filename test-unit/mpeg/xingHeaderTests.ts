...
@suite
class Mpeg_XingHeaderTests {
	@test
	public fromInfo_invalidParameters() {
		// Act / Assert
		Testers.testUint((v: number) => {
			XingHeader.fromInfo(v, 123);
		});
		Testers.testUint((v: number) => {
			XingHeader.fromInfo(123, v);
		});
	}
	...
	public fromData_invalidParameters() {
		// Act / Assert
		assert.throws(() => {
			XingHeader.fromData(undefined);
		});
		assert.throws(() => {
			XingHeader.fromData(null);
		});
	}
	...
	public fromData_zeroTotalFrames() {
		// Arrange
		const data = ByteVector.concatenate(XingHeader.FILE_IDENTIFIER, 0x00, 0x00, 0x00, 0x02, ByteVector.fromUint(123));
	...
	public fromData_zeroTotalSize() {
		// Arrange
		const data = ByteVector.concatenate(XingHeader.FILE_IDENTIFIER, 0x00, 0x00, 0x00, 0x01, ByteVector.fromUint(123));
	...
	public fromData_totalSizeAndFrames() {
		// Arrange
		const data = ByteVector.concatenate(XingHeader.FILE_IDENTIFIER, 0x00, 0x00, 0x00, 0x03, ByteVector.fromUint(123), ByteVector.fromUint(234));
