...
class Matroska_TagValueTests {
	...
	public fromSimpleTagElement_stringValueDefaultLanguage() {
		...
		const bytes = [
			...
			0x44,
			0x84, // Identifier (TAG_DEFAULT)
			0x81, // Size (1)
			0x01, // Value (true)

			0x44,
			0x87, // Identifier (TAG_STRING)
			0x83, // Size (3)
			0x42,
			0x43,
			0x44, // Value (BCD)
	...
	public fromSimpleTagElement_bcp47LanguageCode() {
		...
		const bytes = [
			...
			0x44,
			0x7b, // Identifier (TAG_LANGUAGE_BCP47)
			0x83, // Size (3)
			0x65,
			0x6e,
			0x67, // Value (eng)
