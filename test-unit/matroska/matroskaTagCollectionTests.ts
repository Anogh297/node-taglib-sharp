...
class Matroska_TagCollectionTests {
	...
	@test
	public title() {
		this.testStringTag((c) => c.title, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "TITLE");
	}

	@test
	public titleSort() {
		this.testStringTag((c) => c.titleSort, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "TITLE:SORT_WITH");
	}

	@test
	public subtitle() {
		this.testStringTag((c) => c.subtitle, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "SUBTITLE");
	}

	@test
	public description() {
		this.testStringTag((c) => c.description, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "SUMMARY");
	}
	...
	public performers() {
		this.testStringArrayTag(
			...
			[MatroskaTagTargetType.TRACK, MatroskaTagTargetType.ALBUM_PART, MatroskaTagTargetType.ALBUM, MatroskaTagTargetType.VOLUME, MatroskaTagTargetType.COLLECTION],
			"ACTOR",
			"PERFORMER",
		);
	}
	...
	@test
	public albumArtists() {
		this.testStringArrayTag((c) => c.albumArtists, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "ARTIST");
	}

	@test
	public albumArtistsSort() {
		this.testStringArrayTag((c) => c.albumArtistsSort, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "ARTIST:SORT_WITH");
	}

	@test
	public composers() {
		this.testStringArrayTag((c) => c.composers, [MatroskaTagTargetType.EPISODE], [MatroskaTagTargetType.TRACK], "COMPOSER");
	}

	@test
	public composersSort() {
		this.testStringArrayTag((c) => c.composersSort, [MatroskaTagTargetType.EPISODE], [MatroskaTagTargetType.TRACK], "COMPOSER:SORT_WITH");
	}

	public album() {
		this.testStringTag((c) => c.album, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "TITLE");
	}

	@test
	public albumSort() {
		this.testStringTag((c) => c.albumSort, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "TITLE:SORT_WITH");
	}
	...
	public genres() {
		...
		const videoTargets = [MatroskaTagTargetType.EPISODE, MatroskaTagTargetType.SEQUEL, MatroskaTagTargetType.COLLECTION];
	...
	@test
	public track() {
		this.testNumberTag((c) => c.track, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "PART_NUMBER");
	}

	@test
	public trackCount() {
		this.testNumberTag((c) => c.trackCount, [MatroskaTagTargetType.VOLUME], [MatroskaTagTargetType.ALBUM_PART], "TOTAL_PARTS");
	}

	@test
	public disc() {
		this.testNumberTag((c) => c.disc, [MatroskaTagTargetType.VOLUME], [MatroskaTagTargetType.ALBUM_PART], "PART_NUMBER");
	}

	@test
	public discCount() {
		this.testNumberTag((c) => c.discCount, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "TOTAL_PARTS");
	}

	@test
	public lyrics() {
		this.testStringTag((c) => c.lyrics, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "LYRICS");
	}

	@test
	public grouping() {
		this.testStringTag((c) => c.grouping, [MatroskaTagTargetType.COLLECTION], [MatroskaTagTargetType.ALBUM], "GROUPING");
	}

	@test
	public beatsPerMinute() {
		this.testNumberTag((c) => c.beatsPerMinute, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "BPM");
	}

	@test
	public conductor() {
		this.testStringTag((c) => c.conductor, [MatroskaTagTargetType.EPISODE, undefined], [MatroskaTagTargetType.TRACK, undefined], "DIRECTOR", "CONDUCTOR");
	}
	...
	private testStringArrayTag(
		...
		this.testTagArray<string>(["foo", "bar", "baz"], propertyFunc, ["foo", "bar", "baz"], videoTargets, audioTargets, videoKey, audioKey);
	}
	...
	private getTestTagValue(key: string, value: string): MatroskaTagValue {
		...
		if (keySplit.length > 1) {
			...
			result.nestedTags = [MatroskaTagValue.fromValue(4, keySplit[1], value)];
		} else {
	...
	private runTagArrayTest<TValue>(
		...
		// Arrange
		const targetObj = targetType ? MatroskaTagTarget.fromEmpty(targetType) : MatroskaTagTarget.fromEmpty();
		...
		assert.sameOrderedMembers(
			actualValue,
			expectedValues,
			`Failed isVideo:${isVideo ? "true" : "false"} ` + `values:${tagValues.map((v) => v.value).join(";")} ` + `level:${targetType?.value}`,
		);
	}
	...
	private runTagTest<TValue>(
		...
		if (actualValue instanceof Date && expectedValue instanceof Date) {
			...
			assert.strictEqual(actualValue, expectedValue, `Failed isVideo:${isVideo ? "true" : "false"} value:${tagValue.value}, level:${targetType?.value}`);
		}
	}
