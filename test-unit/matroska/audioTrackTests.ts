...
@suite
class Matroska_AudioTrackTests {
	...
	private BASE_AUDIO_ELEMENTS = new Map<number, EbmlElement>([
		...
		[
			...
			MatroskaTestUtils.getTestElement(ByteVector.fromByteArray([0x40, 0x0b, 0xa5, 0xe3, 0x53, 0xf7, 0xce, 0xd9]), MatroskaIds.SAMPLING_FREQ),
		],
	]);
	...
	public constructor_wrongTrackType(): void {
		...
		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Video, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new AudioTrack(map, map));
