...
@suite
class Matroska_VideoTrackTests {
	private BASE_TRACK_ELEMENTS = new Map<number, EbmlElement>([
		...
		[MatroskaIds.CODEC_PRIVATE, MatroskaTestUtils.getTestElement(ByteVector.fromByteArray([0xaa, 0xbb, 0xcc]), MatroskaIds.CODEC_PRIVATE)],
	...
	private BASE_VIDEO_ELEMENTS = new Map<number, EbmlElement>([
		...
		[MatroskaIds.DISPLAY_UNIT, MatroskaTestUtils.getTestElement(VideoDisplayUnits.Centimeters, MatroskaIds.DISPLAY_UNIT)],
	...
	public constructor_wrongTrackType(): void {
		...
		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Audio, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Buttons, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Logo, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Complex, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Control, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Metadata, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));

		map.set(MatroskaIds.TRACK_TYPE, MatroskaTestUtils.getTestElement(MatroskaTrackType.Subtitle, MatroskaIds.TRACK_TYPE));
		assert.throws(() => new VideoTrack(map, map));
	}
	...
	public constructor_interlacingModeOutsideRange(): void {
		...
		const videoElements = new Map<number, EbmlElement>([[MatroskaIds.FLAG_INTERLACED, MatroskaTestUtils.getTestElement(1234, MatroskaIds.FLAG_INTERLACED)]]);
	...
	public constructor_interlacingModeInsideRange(): void {
		...
		const videoElements = new Map<number, EbmlElement>([
			[MatroskaIds.FLAG_INTERLACED, MatroskaTestUtils.getTestElement(VideoInterlaceFlag.Progressive, MatroskaIds.FLAG_INTERLACED)],
		]);
