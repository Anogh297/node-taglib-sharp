...
class Ogg_PageHeaderTests {
	...
	public fromFile_unsupportedSize() {
		// Arrange
		const data = ByteVector.concatenate(OggPageHeader.HEADER_IDENTIFIER, 0x05, 0x06, ByteVector.fromUlong(BigInt(0x100000000000000)));
		const file = TestFile.getFile(data);
	...
	public fromFile_invalidPageSegmentCount() {
		...
		const data = ByteVector.concatenate(
			...
			0x00,
			0x00,
			0x00, // Absolute granular position
	...
	public fromFile_noPacketsEndInFile() {
		...
		const data = ByteVector.concatenate(
			...
			0xff,
			0xff,
			0xff, // Absolute granular position
			ByteVector.fromUint(0x1234, false), // Stream serial number
			ByteVector.fromUint(0x2345, false), // Page sequence number
			ByteVector.fromSize(4), // Checksum
	...
	public setPacketSizes_invalidParameters() {
		...
		// Act / Assert
		Testers.testTruthy((v: number[]) => {
			header.packetSizes = v;
		});
		assert.throws(() => {
			header.packetSizes = [123, undefined];
		});
	}
