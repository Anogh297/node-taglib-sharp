...
class Ogg_VorbisTests {
	...
	public readPacket_packetIsCommentData() {
		// Arrange
		const codec = Ogg_VorbisTests.getTestCodec();
		const packet = ByteVector.concatenate(0x03, ByteVector.fromString("vorbis", StringType.UTF8), ByteVector.fromString("foobarbaz", StringType.UTF8));
	...
	public readPacket_commentsAlreadyRead() {
		// Arrange
		const codec = Ogg_VorbisTests.getTestCodec();
		const commentPacket1 = ByteVector.concatenate(0x03, ByteVector.fromString("vorbis", StringType.UTF8), ByteVector.fromString("foobarbaz", StringType.UTF8));
		const commentPacket2 = ByteVector.concatenate(0x03, ByteVector.fromString("vorbis", StringType.UTF8), ByteVector.fromString("fuxbuxquxx", StringType.UTF8));
	...
	public writeCommentPacket_noPackets() {
		...
		const expected = ByteVector.concatenate(0x03, ByteVector.fromString("vorbis", StringType.UTF8), comment.render(true));
	...
	public writeCommentPacket_hasCommentPacket() {
		...
		const codec = Ogg_VorbisTests.getTestCodec();
		const comment = XiphComment.fromEmpty();
		const packets = [ByteVector.fromSize(10, 0x0c), ByteVector.concatenate(0x03, ByteVector.fromString("vorbis", StringType.UTF8)), ByteVector.fromSize(10, 0x0f)];
