...
class ByteVector_ConversionTests {
	private readonly doublePositiveBV = ByteVector.fromByteArray(
		// 56.12
		new Uint8Array([0x8f, 0xc2, 0xf5, 0x28, 0x5c, 0x0f, 0x4c, 0x40, 0xaa]),
	);
	private readonly doubleNegativeBV = ByteVector.fromByteArray(
		// -12.34
		new Uint8Array([0xae, 0x47, 0xe1, 0x7a, 0x14, 0xae, 0x28, 0xc0, 0xaa]),
	);
	private readonly floatPositiveBV = ByteVector.fromByteArray(
		// 56.12
		new Uint8Array([0xe1, 0x7a, 0x60, 0x42, 0xaa]),
	);
	private readonly floatNegativeBV = ByteVector.fromByteArray(
		// -12.34
		new Uint8Array([0xa4, 0x70, 0x45, 0xc1, 0xaa]),
	);
	private readonly intNegativeCompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfe, 0xfd, 0xfc, 0xfc, 0xaa]));
	private readonly intNegativeIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfc, 0xfd]));
	private readonly intPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04, 0xaa]));
	private readonly intPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02]));
	private readonly longNegativeCompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfe, 0xfd, 0xfc, 0xfb, 0xfa, 0xf9, 0xf8, 0xf8, 0xaa]));
	private readonly longNegativeIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfe, 0xfd, 0xfc, 0xfc]));
	private readonly longPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0xaa]));
	private readonly longPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04]));
	private readonly shortNegativeCompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfe, 0xfd, 0xaa]));
	private readonly shortNegativeIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0xfc]));
	private readonly shortPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0xaa]));
	private readonly shortPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01]));
	private readonly uintPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04, 0xaa]));
	private readonly uintPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02]));
	private readonly ulongPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0xaa]));
	private readonly ulongPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0x03, 0x04]));
	private readonly ushortPositiveCompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01, 0x02, 0xaa]));
	private readonly ushortPositiveIncompleteBV = ByteVector.fromByteArray(new Uint8Array([0x01]));
	...
	public toBase64String_view() {
		// Arrange
		const bv = ByteVector.concatenate(0xaa, ByteVector.fromString(TestConstants.testFileContentsStr, StringType.Latin1), 0xaa);
	...
	public toDouble_invalidSize() {
		assert.throws(() => {
			ByteVector.fromSize(0).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(1).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(2).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(3).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(4).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(5).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(6).toDouble();
		});
		assert.throws(() => {
			ByteVector.fromSize(7).toDouble();
		});
	}

	@test
	public toDouble_zero_complete() {
		const double = ByteVector.fromByteArray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).toDouble();
		assert.strictEqual(double, 0);
	}

	@test
	public toDouble_zero_completeView() {
		const double = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 8).toDouble();
		assert.strictEqual(double, 0);
	}
	...
	public toDouble_positiveBigEndianView() {
		const double = ByteVector.concatenate(0xaa, this.doublePositiveBV, 0xaa).subarray(1, this.doublePositiveBV.length).toDouble();
		assert.approximately(double, -9.5397675953257207e-233, 0.000000000001e-233);
	}
	...
	public toDouble_positiveLittleEndianView() {
		const double = ByteVector.concatenate(0xaa, this.doublePositiveBV, 0xaa).subarray(1, this.doublePositiveBV.length).toDouble(false);
		assert.approximately(double, 56.12, 0.01);
	}
	...
	public toDouble_negativeBigEndianView() {
		const double = ByteVector.concatenate(0xaa, this.doubleNegativeBV, 0xaa).subarray(1, this.doubleNegativeBV.length).toDouble();
		assert.approximately(double, -9.6037214055410557e-86, 0.00000000001e-86);
	}
	...
	public toDouble_negativeLittleEndianView() {
		const double = ByteVector.concatenate(0xaa, this.doubleNegativeBV, 0xaa).subarray(1, this.doubleNegativeBV.length).toDouble(false);
		assert.approximately(double, -12.34, 0.01);
	}

	@test
	public toDouble_doubleRangeBigEndian() {
		const double = ByteVector.fromByteArray([0x7f, 0xef, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]).toDouble();
		assert.approximately(double, 1.7976931348623157e308, 0.000000000001e308);
	}

	@test
	public toDouble_doubleRangeLittleEndian() {
		const double = ByteVector.fromByteArray([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xef, 0x7f]).toDouble(false);
		assert.approximately(double, 1.7976931348623157e308, 0.000000000001e308);
	}
	...
	public toFloat_invalidSize() {
		assert.throws(() => {
			ByteVector.fromSize(0).toFloat();
		});
		assert.throws(() => {
			ByteVector.fromSize(1).toFloat();
		});
		assert.throws(() => {
			ByteVector.fromSize(2).toFloat();
		});
		assert.throws(() => {
			ByteVector.fromSize(3).toFloat();
		});
	}
	...
	public toFloat_zero_completeView() {
		const double = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 4).toFloat();
		assert.strictEqual(double, 0);
	}
	...
	public toFloat_positiveBigEndianView() {
		const float = ByteVector.concatenate(0xaa, this.floatPositiveBV, 0xaa).subarray(1, this.floatPositiveBV.length).toFloat();
		assert.approximately(float, -2.88663883e20, 0.00000001e20);
	}
	...
	public toFloat_positiveLittleEndianView() {
		const float = ByteVector.concatenate(0xaa, this.floatPositiveBV, 0xaa).subarray(1, this.floatPositiveBV.length).toFloat(false);
		assert.approximately(float, 56.12, 0.01);
	}
	...
	public toFloat_negativeBigEndianView() {
		const float = ByteVector.concatenate(0xaa, this.floatNegativeBV, 0xaa).subarray(1, this.floatNegativeBV.length).toFloat();
		assert.approximately(float, -5.21007881e-17, 0.00000001e-17);
	}
	...
	public toFloat_negativeLittleEndianView() {
		const float = ByteVector.concatenate(0xaa, this.floatNegativeBV, 0xaa).subarray(1, this.floatNegativeBV.length).toFloat(false);
		assert.approximately(float, -12.34, 0.01);
	}
	...
	public toInt_emptyView() {
		const uint = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toInt();
		assert.strictEqual(uint, 0);
	}

	@test
	public toInt_zero_complete() {
		const int = ByteVector.fromByteArray([0x0, 0x0, 0x0, 0x0, 0xaa]).toInt();
		assert.strictEqual(int, 0);
	}

	@test
	public toInt_zero_completeView() {
		const int = ByteVector.fromByteArray([0xaa, 0x0, 0x0, 0x0, 0x0, 0xaa]).subarray(1, 4).toInt();
		assert.strictEqual(int, 0);
	}

	@test
	public toInt_zero_incomplete() {
		const int = ByteVector.fromByteArray([0x0, 0x0]).toInt();
		assert.strictEqual(int, 0);
	}

	@test
	public toInt_zero_incompleteView() {
		const int = ByteVector.fromByteArray([0xaa, 0x0, 0x0, 0xaa]).subarray(1, 2).toInt();
		assert.strictEqual(int, 0);
	}
	...
	public toInt_positiveBigEndian_completeView() {
		const int = ByteVector.concatenate(0xaa, this.intPositiveCompleteBV, 0xaa).subarray(1, 4).toInt();
		assert.strictEqual(int, 0x01020304);
	}
	...
	public toInt_positiveBigEndian_incompleteView() {
		const int = ByteVector.concatenate(0xaa, this.intPositiveIncompleteBV, 0xaa).subarray(1, 2).toInt();
		assert.strictEqual(int, 0x00000102);
	}
	...
	public toInt_positiveLittleEndian_completeView() {
		const int = ByteVector.concatenate(0xaa, this.intPositiveCompleteBV, 0xaa).subarray(1, 4).toInt(false);
		assert.strictEqual(int, 0x04030201);
	}
	...
	public toInt_positiveLittleEndian_incompleteView() {
		const int = ByteVector.concatenate(0xaa, this.intPositiveIncompleteBV, 0xaa).subarray(1, 2).toInt(false);
		assert.strictEqual(int, 0x00000201);
	}
	...
	public toInt_negativeBigEndian_completeView() {
		const int = ByteVector.concatenate(0xaa, this.intNegativeCompleteBV, 0xaa).subarray(1, 4).toInt(true);
		assert.strictEqual(int, -0x01020304);
	}
	...
	public toInt_negativeBigEndian_incompleteView() {
		const int = ByteVector.concatenate(0xaa, this.intNegativeIncompleteBV, 0xaa).subarray(1, 2).toInt(true);
		assert.strictEqual(int, 0x0000fcfd);
	}
	...
	public toInt_negativeLittleEndian_completeView() {
		const int = ByteVector.concatenate(0xaa, this.intNegativeCompleteBV, 0xaa).subarray(1, 4).toInt(false);
		assert.strictEqual(int, -0x03030202);
	}
	...
	public toInt_negativeLittleEndian_incompleteView() {
		const int = ByteVector.concatenate(0xaa, this.intNegativeIncompleteBV, 0xaa).subarray(1, 2).toInt(false);
		assert.strictEqual(int, 0x0000fdfc);
	}
	...
	public toLong_emptyView() {
		const ulong = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toLong();
		assert.strictEqual(ulong, BigInt(0));
	}

	@test
	public toLong_zero_complete() {
		const long = ByteVector.fromByteArray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa]).toLong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toLong_zero_completeView() {
		const long = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 8).toLong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toLong_zero_incomplete() {
		const long = ByteVector.fromByteArray([0x00, 0x00, 0x00, 0x00]).toLong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toLong_zero_incompleteView() {
		const long = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 4).toLong();
		assert.strictEqual(long, BigInt(0));
	}
	...
	public toLong_positiveBigEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.longPositiveCompleteBV, 0xaa).subarray(1, this.longPositiveCompleteBV.length).toLong();
		assert.strictEqual(long, BigInt("0x0102030405060708"));
	}
	...
	public toLong_positiveBigEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.longPositiveIncompleteBV, 0xaa).subarray(1, this.longPositiveIncompleteBV.length).toLong();
		assert.strictEqual(long, BigInt("0x01020304"));
	}
	...
	public toLong_positiveLittleEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.longPositiveCompleteBV, 0xaa).subarray(1, this.longPositiveCompleteBV.length).toLong(false);
		assert.strictEqual(long, BigInt("0x0807060504030201"));
	}
	...
	public toLong_positiveLittleEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.longPositiveIncompleteBV, 0xaa).subarray(1, this.longPositiveIncompleteBV.length).toLong(false);
		assert.strictEqual(long, BigInt("0x04030201"));
	}
	...
	public toLong_negativeBigEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.longNegativeCompleteBV, 0xaa).subarray(1, this.longNegativeCompleteBV.length).toLong();
		assert.strictEqual(long, BigInt("-72623859790382856"));
	}
	...
	public toLong_negativeBigEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.longNegativeIncompleteBV, 0xaa).subarray(1, this.longNegativeIncompleteBV.length).toLong();
		assert.strictEqual(long, BigInt("0xFEFDFCFC"));
	}
	...
	public toLong_negativeLittleEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.longNegativeCompleteBV, 0xaa).subarray(1, this.longNegativeCompleteBV.length).toLong(false);
		assert.strictEqual(long, BigInt("-506380101714379266"));
	}
	...
	public toLong_negativeLittleEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.longNegativeIncompleteBV, 0xaa).subarray(1, this.longNegativeIncompleteBV.length).toLong(false);
		assert.strictEqual(long, BigInt("0xFCFCFDFE"));
	}
	...
	public toShort_emptyView() {
		const short = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toShort();
		assert.strictEqual(short, 0);
	}

	@test
	public toShort_zero_complete() {
		const int = ByteVector.fromByteArray([0x0, 0x0, 0xaa]).toShort();
		assert.strictEqual(int, 0);
	}

	@test
	public toShort_zero_completeView() {
		const short = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0xaa]).subarray(1, 2).toShort();
		assert.strictEqual(short, 0);
	}

	@test
	public toShort_zero_incomplete() {
		const int = ByteVector.fromByteArray([0x0]).toShort();
		assert.strictEqual(int, 0);
	}

	@test
	public toShort_zero_incompleteView() {
		const short = ByteVector.fromByteArray([0xaa, 0x00, 0xaa]).subarray(1, 1).toShort();
		assert.strictEqual(short, 0);
	}
	...
	public toShort_positiveBigEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.shortPositiveCompleteBV, 0xaa).subarray(1, this.shortPositiveCompleteBV.length).toShort();
	...
	public toShort_positiveBigEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.shortPositiveIncompleteBV, 0xaa).subarray(1, this.shortPositiveIncompleteBV.length).toShort();
	...
	public toShort_positiveLittleEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.shortPositiveCompleteBV, 0xaa).subarray(1, this.shortPositiveCompleteBV.length).toShort(false);
	...
	public toShort_negativeBigEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.shortNegativeCompleteBV, 0xaa).subarray(1, this.shortNegativeCompleteBV.length).toShort();
	...
	public toShort_negativeBigEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.shortNegativeIncompleteBV, 0xaa).subarray(1, this.shortNegativeIncompleteBV.length).toShort();
	...
	public toShort_negativeLittleEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.shortNegativeCompleteBV, 0xaa).subarray(1, this.shortNegativeCompleteBV.length).toShort(false);
	...
	public toShort_negativeLittleEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.shortNegativeIncompleteBV, 0xaa).subarray(1, this.shortNegativeIncompleteBV.length).toShort(false);
		assert.strictEqual(short, 0x00fc);
	}
	...
	public toString_invalidOffset() {
		// Arrange, Act, Assert
		assert.throws(() => {
			ByteVector.fromSize(0).toString(0.1);
		});
		assert.throws(() => {
			ByteVector.fromSize(0).toString(-1);
		});
		assert.throws(() => {
			ByteVector.fromSize(0).toString(1234);
		});
	}

	@test
	public toString_utf8Full() {
		const str = ByteVector.fromString(TestConstants.testStrings.UTF8.str, StringType.UTF8).toString(StringType.UTF8);
		assert.strictEqual(str, TestConstants.testStrings.UTF8.str);
	}

	@test
	public toString_utf8Empty() {
		const str = ByteVector.fromSize(0).toString(StringType.UTF8);
	...
	public toString_utf16LittleEndianFull() {
		...
		const str = ByteVector.fromString(TestConstants.testStrings.UTF16LE.str, StringType.UTF16LE).toString(StringType.UTF16LE);
	...
	public toString_utf16BigEndianFull() {
		const originalLastUtf16Encoding = Encoding["_lastUtf16Encoding"];
		const str = ByteVector.fromString(TestConstants.testStrings.UTF16BE.str, StringType.UTF16BE).toString(StringType.UTF16BE);
	...
	public toString_latin1Full() {
		const str = ByteVector.fromString(TestConstants.testStrings.Latin1.str, StringType.Latin1).toString(StringType.Latin1);
	...
	public toString_utf16Full() {
		...
		try {
			const str = ByteVector.fromString(TestConstants.testStrings.UTF16LEWithBOM.str, StringType.UTF16).toString(StringType.UTF16);
	...
	public toStrings_invalidCount() {
		// Arrange, Act, Assert
		assert.throws(() => {
			ByteVector.fromSize(1).toStrings(undefined, 0.1);
		});
		assert.throws(() => {
			ByteVector.fromSize(1).toStrings(undefined, -1);
		});
	}
	...
	public toStrings_dangling() {
		// Arrange
		const bv = ByteVector.concatenate(TestConstants.testStrings.UTF8.bytes, 0x00, TestConstants.testStrings.UTF8.bytes);
	...
	public toStrings_maxCount() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF8.str, TestConstants.testStrings.UTF8.str], StringType.UTF8, 1, [
	...
	public toStrings_utf8Single() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF8.str], StringType.UTF8, undefined, [TestConstants.testStrings.UTF8.str]);
	}

	@test
	public toStrings_utf8Multiple() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF8.str, TestConstants.testStrings.UTF8.str], StringType.UTF8, undefined, [
			TestConstants.testStrings.UTF8.str,
			TestConstants.testStrings.UTF8.str,
		]);
	}
	...
	public toStrings_utf16LittleEndianSingle() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF16LE.str], StringType.UTF16LE, undefined, [TestConstants.testStrings.UTF16LE.str]);
	}

	@test
	public toStrings_utf16LittleEndianMultiple() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF16LE.str, TestConstants.testStrings.UTF16LE.str], StringType.UTF16LE, undefined, [
	...
	public toStrings_utf16BigEndianSingle() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF16BE.str], StringType.UTF16BE, undefined, [TestConstants.testStrings.UTF16BE.str]);
	}

	@test
	public toStrings_utf16BigEndianMultiple() {
		ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF16BE.str, TestConstants.testStrings.UTF16BE.str], StringType.UTF16BE, undefined, [
	...
	public toStrings_utf16Single() {
		...
		try {
			ByteVector_ConversionTests.testStrings([TestConstants.testStrings.UTF16LEWithBOM.str], StringType.UTF16, undefined, [
	...
	public toUint_emptyView() {
		const uint = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toUint();
		assert.strictEqual(uint, 0);
	}

	@test
	public toUint_zero_complete() {
		const int = ByteVector.fromByteArray([0x0, 0x0, 0x0, 0x0, 0xaa]).toUint();
		assert.strictEqual(int, 0);
	}

	@test
	public toUint_zero_completeView() {
		const short = ByteVector.fromByteArray([0xaa, 0x0, 0x0, 0x0, 0x0, 0xaa]).subarray(1, 4).toShort();
		assert.strictEqual(short, 0);
	}

	@test
	public toUint_zero_incomplete() {
		const int = ByteVector.fromByteArray([0x0, 0x0]).toUint();
		assert.strictEqual(int, 0);
	}

	@test
	public toUint_zero_incompleteView() {
		const short = ByteVector.fromByteArray([0xaa, 0x0, 0x0, 0xaa]).subarray(1, 2).toShort();
		assert.strictEqual(short, 0);
	}
	...
	public toUint_positiveBigEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.uintPositiveCompleteBV, 0xaa).subarray(1, this.uintPositiveCompleteBV.length).toUint();
	...
	public toUint_positiveBigEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.uintPositiveIncompleteBV, 0xaa).subarray(1, this.uintPositiveIncompleteBV.length).toUint();
	...
	public toUint_positiveLittleEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.uintPositiveCompleteBV, 0xaa).subarray(1, this.uintPositiveCompleteBV.length).toUint(false);
	...
	public toUint_positiveLittleEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.uintPositiveIncompleteBV, 0xaa).subarray(1, this.uintPositiveIncompleteBV.length).toShort(false);
		assert.strictEqual(short, 0x00000201);
	}

	@test
	public toUint_unsignedRange_complete() {
		const int = ByteVector.fromByteArray([0xff, 0xff, 0xff, 0xff]).toUint();
	...
	public toUlong_emptyView() {
		const ulong = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toUlong();
		assert.strictEqual(ulong, BigInt(0));
	}

	@test
	public toUlong_zero_complete() {
		const long = ByteVector.fromByteArray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa]).toUlong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toUlong_zero_completeView() {
		const long = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 8).toUlong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toUlong_zero_incomplete() {
		const long = ByteVector.fromByteArray([0x00, 0x00, 0x00, 0x00]).toUlong();
		assert.strictEqual(long, BigInt(0));
	}

	@test
	public toUlong_zero_incompleteView() {
		const long = ByteVector.fromByteArray([0xaa, 0x00, 0x00, 0x00, 0x00, 0xaa]).subarray(1, 4).toUlong();
		assert.strictEqual(long, BigInt(0));
	}
	...
	public toUlong_positiveBigEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.ulongPositiveCompleteBV, 0xaa).subarray(1, this.ulongPositiveCompleteBV.length).toUlong();
		assert.strictEqual(long, BigInt("0x0102030405060708"));
	}
	...
	public toUlong_positiveBigEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.ulongPositiveIncompleteBV, 0xaa).subarray(1, this.ulongPositiveIncompleteBV.length).toUlong();
		assert.strictEqual(long, BigInt("0x01020304"));
	}
	...
	public toUlong_positiveLittleEndian_completeView() {
		const long = ByteVector.concatenate(0xaa, this.ulongPositiveCompleteBV, 0xaa).subarray(1, this.ulongPositiveCompleteBV.length).toUlong(false);
		assert.strictEqual(long, BigInt("0x0807060504030201"));
	}
	...
	public toUlong_positiveLittleEndian_incompleteView() {
		const long = ByteVector.concatenate(0xaa, this.ulongPositiveIncompleteBV, 0xaa).subarray(1, this.ulongPositiveIncompleteBV.length).toUlong(false);
		assert.strictEqual(long, BigInt("0x04030201"));
	}

	@test
	public toUlong_unsignedRange_complete() {
		const long = ByteVector.fromByteArray(new Uint8Array([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff])).toUlong();
	...
	public toUshort_emptyView() {
		const short = ByteVector.fromByteArray([0xaa, 0xaa]).subarray(1, 0).toUshort();
		assert.strictEqual(short, 0);
	}

	@test
	public toUshort_zero_complete() {
		const int = ByteVector.fromByteArray([0x0, 0x0, 0xaa]).toUshort();
		assert.strictEqual(int, 0);
	}

	@test
	public toUshort_zero_completeView() {
		const short = ByteVector.fromByteArray([0xaa, 0x0, 0x0, 0xaa]).subarray(1, 2).toUshort();
		assert.strictEqual(short, 0);
	}

	@test
	public toUshort_zero_incomplete() {
		const int = ByteVector.fromByteArray([0x0]).toUshort();
		assert.strictEqual(int, 0);
	}

	@test
	public toUshort_zero_incompleteView() {
		const short = ByteVector.fromByteArray([0xaa, 0x00, 0xaa]).subarray(1, 1).toUshort();
		assert.strictEqual(short, 0);
	}
	...
	public toUshort_positiveBigEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.ushortPositiveCompleteBV, 0xaa).subarray(1, this.ushortPositiveCompleteBV.length).toUshort();
		assert.strictEqual(short, 0x0102);
	}
	...
	public toUshort_positiveBigEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.ushortPositiveIncompleteBV, 0xaa).subarray(1, this.ushortPositiveIncompleteBV.length).toUshort();
		assert.strictEqual(short, 0x01);
	}
	...
	public toUshort_positiveLittleEndian_completeView() {
		const short = ByteVector.concatenate(0xaa, this.ushortPositiveCompleteBV, 0xaa).subarray(1, this.ushortPositiveCompleteBV.length).toUshort(false);
		assert.strictEqual(short, 0x0201);
	}
	...
	public toUshort_positiveLittleEndian_incompleteView() {
		const short = ByteVector.concatenate(0xaa, this.ushortPositiveIncompleteBV, 0xaa).subarray(1, this.ushortPositiveIncompleteBV.length).toUshort(false);
		assert.strictEqual(short, 0x01);
	}
	...
	private static testStrings(textInput: string[], stringType: StringType, count: number, expected: string[]) {
		// Arrange
