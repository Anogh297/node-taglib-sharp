...
class Id3v2_TermsOfUseFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromOffsetRawData_notEnoughBytes() {
		...
		// Act / Assert
		assert.throws(() => {
			TermsOfUseFrame.fromOffsetRawData(data, 2, header, 4);
		});
	}
	...
	public fromRawData_notEnoughBytes() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.USER);
		header.frameSize = 2;
		const data = ByteVector.concatenate(header.render(4), 0x01, 0x02);

		// Act / Assert
		assert.throws(() => {
			TermsOfUseFrame.fromRawData(data, 4);
		});
	}
...
class Id3v2_TermsOfUseFrame_MethodTests {
	...
	public find_falsyFrames() {
		// Act/Assert
		assert.throws(() => {
			TermsOfUseFrame.find(undefined);
		});
	}
	...
	public find_matchWithFrames_returnsFirst() {
		// Arrange
		const frames = [TermsOfUseFrame.fromFields("eng"), TermsOfUseFrame.fromFields("jpn")];
