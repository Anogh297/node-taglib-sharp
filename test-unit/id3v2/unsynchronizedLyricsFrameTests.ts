...
class Id3v2_UnsynchronizedLyricsFrame_MethodTests {
	...
	public find_falsyFrames_throws() {
		...
		Testers.testTruthy((v: UnsynchronizedLyricsFrame[]) => {
			UnsynchronizedLyricsFrame.find(v, "foo", "bar");
		});
	}
	...
	public find_matchWithLanguage_returnsFirstMatch() {
		// Arrange
		const frames = [getTestUnsynchronizedLyricsFrame(), getTestUnsynchronizedLyricsFrame(), getTestUnsynchronizedLyricsFrame(), getTestUnsynchronizedLyricsFrame()];
	...
	public findAll_falsyFrames_throws() {
		// Act/Assert
		assert.throws(() => {
			UnsynchronizedLyricsFrame.findAll(null, "foo", "bar");
		});
		assert.throws(() => {
			UnsynchronizedLyricsFrame.findAll(undefined, "foo", "bar");
		});
	}
