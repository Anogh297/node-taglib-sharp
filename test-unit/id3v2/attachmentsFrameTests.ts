...
const getTestFrame = () => getCustomTestFrame(ByteVector.fromString("foobarbaz", StringType.UTF8), "fux", "bux", "application/octet-stream", PictureType.FrontCover);

const getCustomTestFrame = (data: ByteVector, desc: string, filename: string, mimeType: string, type: PictureType): AttachmentFrame => {
...
class Id3v2_AttachmentFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromRawData_invalidFrameIdentifier() {
		...
		header.frameSize = 10;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromSize(10));
	...
	public fromRawData_dataTooShort() {
		...
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromSize(3, 0x00));
	...
	private static verifyFrame(frame: AttachmentFrame, ft: FrameIdentifier, d: ByteVector, desc: string, fn: string, mt: string, te: StringType, t: PictureType) {
		assert.isOk(frame);
...
class Id3v2_AttachmentFrame_MethodTests {
	...
	public find_falsyFrames() {
		// Act / Assert
		assert.throws(() => {
			AttachmentFrame.find(undefined);
		});
		assert.throws(() => {
			AttachmentFrame.find(null);
		});
	}
