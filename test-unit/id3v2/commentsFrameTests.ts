...
class Id3v2_CommentsFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromDescription_falsyDescription() {
		// Act/Assert
		Testers.testTruthy((v: string) => {
			CommentsFrame.fromDescription(v, undefined, undefined);
		});
	}
	...
	public fromDescription_withoutLanguage() {
		...
		// Assert
		Id3v2_CommentsFrame_ConstructorTests.validateFrame(frame, description, "XXX", Id3v2Settings.defaultEncoding, "");
	}
	...
	public fromOffsetRawData_tooFewBytes_throws() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.COMM);
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, StringType.Latin1);

		// Act/Assert
		assert.throws(() => {
			CommentsFrame.fromOffsetRawData(data, 2, header, 4);
		});
	}
	...
	public fromOffsetRawData_noData_returnsEmptyFrame() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.COMM);
		header.frameSize = 4;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, StringType.Latin1, ByteVector.fromString("eng", StringType.Latin1));
	...
	public fromRawData_tooFewBytes_throws() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.COMM);
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(4), StringType.Latin1);

		// Act/Assert
		assert.throws(() => {
			CommentsFrame.fromRawData(data, 4);
		});
	}
	...
	public fromRawData_noData_returnsEmptyFrame() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.COMM);
		header.frameSize = 4;
		const data = ByteVector.concatenate(header.render(4), StringType.Latin1, ByteVector.fromString("eng", StringType.Latin1));
	...
	private static validateFrame(frame: CommentsFrame, expectedDesc: string, expectedLang: string, expectedEncoding: StringType, expectedText: string) {
		assert.isOk(frame);
...
class Id3v2_CommentsFrame_MethodTests {
	...
	public find_falsyFrames() {
		// Act/Assert
		Testers.testTruthy((v: CommentsFrame[]) => {
			CommentsFrame.find(v, "fux");
		});
	}
	...
	public findAll_frameDoesNotExist() {
		// Arrange
		const frames = [CommentsFrame.fromDescription("fux", "jpn"), CommentsFrame.fromDescription("bux", "jpn"), CommentsFrame.fromDescription("qux", "eng")];
	...
	public findPreferred_firstFrameMatch() {
		// Arrange
		const frames = [CommentsFrame.fromDescription("fux", "eng")];
	...
	public findPreferred_languageMatch() {
		// Arrange
		const frames = [CommentsFrame.fromDescription("fux", "eng"), CommentsFrame.fromDescription("bux", "eng"), CommentsFrame.fromDescription("fux", "jpn")];
	...
	public findPreferred_descriptionMatch() {
		// Arrange
		const frames = [CommentsFrame.fromDescription("fux", "eng"), CommentsFrame.fromDescription("bux", "eng")];
