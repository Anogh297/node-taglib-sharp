...
class Id3v2_PlayCountFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromRawData_fourBytePlayCount() {
		...
		header.frameSize = 4;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromUint(1234));
	...
	public fromRawData_sixBytePlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.PCNT);
		header.frameSize = 6;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, ByteVector.fromUint(1234));
	...
	public fromRawData_eightBytePlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.PCNT);
		header.frameSize = 8;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromUlong(BigInt("4294967296")));
	...
	public fromOffsetRawData_twoBytePlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.PCNT);
		header.frameSize = 8;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, ByteVector.fromUlong(BigInt("4294967296")));
...
class Id3v2_PlayCountFrame_MethodTests {
	...
	public render_sixBytePlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.PCNT);
		header.frameSize = 6;
		const data = ByteVector.concatenate(header.render(4), 0x12, 0x34, 0x45, 0x56, 0x78, 0x90);
		const frame = PlayCountFrame.fromRawData(data, 4);
	...
	public render_longBytePlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.PCNT);
		header.frameSize = 8;
		const data = ByteVector.concatenate(header.render(4), 0x12, 0x34, 0x45, 0x56, 0x78, 0x90, 0xab, 0xcd);
		const frame = PlayCountFrame.fromRawData(data, 4);
