...
class Id3v2_UrlLinkFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromOffsetRawData_notUserFrame() {
		...
		const data = ByteVector.concatenate(
			...
			0x00,
			0x00, // Some null bytes to trigger null cleanup
		);
	...
	public fromOffsetRawData_userFrame() {
		...
		// Assert
		Id3v2_UrlLinkFrame_ConstructorTests.assertFrame(output, FrameIdentifiers.WXXX, ["foo", "bar"], StringType.Latin1);
	}
	...
	public fromRawData_notUserFrame() {
		...
		const data = ByteVector.concatenate(
			...
			0x00,
			0x00, // - null bytes to test end byte
			//   cleanup
		);
...
class Id3v2_UrlLinkFrame_MethodTests {
	...
	public findUrlLinkFrame_falsyFrames_throws(): void {
		// Act/Assert
		Testers.testTruthy((v: UrlLinkFrame[]) => {
			UrlLinkFrame.findUrlLinkFrame(v, FrameIdentifiers.WCOM);
		});
	}
	...
	public findUrlLinkFrame_falsyIdentity_throws(): void {
		...
		// Act/Assert
		Testers.testTruthy((v: FrameIdentifier) => {
			UrlLinkFrame.findUrlLinkFrame(frames, v);
		});
	}
