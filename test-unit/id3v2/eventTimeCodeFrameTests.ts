...
class Id3v2_EventTimeCodeTests {
	...
	public time_invalid() {
		...
		assert.throws(() => {
			output.time = Number.MAX_SAFE_INTEGER + 1;
		});
	}
	...
	public render() {
		...
		// Assert
		const expected = ByteVector.concatenate(EventType.KeyChange, ByteVector.fromInt(123));
...
class Id3v2_EventTimeCodeFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromRawData_noEvents() {
		...
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(4), TimestampFormat.AbsoluteMilliseconds);
	...
	public fromRawData_withEvents() {
		...
		const data = ByteVector.concatenate(header.render(4), TimestampFormat.AbsoluteMilliseconds, event1.render(), event2.render());

		// Act
		const frame = EventTimeCodeFrame.fromRawData(data, 4);

		// Assert
		Id3v2_EventTimeCodeFrame_ConstructorTests.assertFrame(frame, [event1, event2], TimestampFormat.AbsoluteMilliseconds);
	}
	...
	public fromOffsetRawData_noEvents() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.ETCO);
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, TimestampFormat.AbsoluteMilliseconds);
	...
	public fromOffsetRawData_withEvents() {
		...
		const event1 = new EventTimeCode(EventType.Profanity, 123);
		const event2 = new EventTimeCode(EventType.KeyChange, 456);
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, TimestampFormat.AbsoluteMilliseconds, event1.render(), event2.render());
...
class Id3v2_EventTimeCodeFrame_MethodTests {
	...
	public render() {
		...
		// Assert
		const expected = ByteVector.concatenate(header.render(4), TimestampFormat.AbsoluteMilliseconds, event1.render(), event2.render());
