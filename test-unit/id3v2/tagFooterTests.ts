...
const getTestFooter = (majorVersion: number, minorVersion: number, flags: Id3v2TagHeaderFlags): Id3v2TagFooter => {
	const data = ByteVector.concatenate(Id3v2TagFooter.FILE_IDENTIFIER, majorVersion, minorVersion, flags, TestConstants.syncedUintBytes);
...
class Id3v2_TagFooter_ConstructorTests {
	...
	public fromData_invalidFlags_version4() {
		// Arrange
		const data = ByteVector.concatenate(Id3v2TagFooter.FILE_IDENTIFIER, 0x04, 0x00, 0x07);
	...
	public fromData_invalidTagSizeBytes() {
		// Arrange
		const testData = ByteVector.concatenate(Id3v2TagFooter.FILE_IDENTIFIER, 0x04, 0x00, 0x00);
	...
	public fromData_validParams() {
		...
		const flags = 0xe0;
		const testData = ByteVector.concatenate(Id3v2TagFooter.FILE_IDENTIFIER, majorVersion, minorVersion, flags, TestConstants.syncedUintBytes);
...
class Id3v2_TagFooter_PropertyTests {
	...
	public setMajorVersion_invalidValues() {
		...
		Testers.testByte((v: number) => {
			footer.majorVersion = v;
		});
		assert.throws(() => {
			footer.majorVersion = 1;
		});
		assert.throws(() => {
			footer.majorVersion = 5;
		});
	}
