...
export default abstract class FrameConstructorTests {
	...
	public fromOffsetRawData_falsyData_throws() {
		...
		Testers.testTruthy((v: ByteVector) => {
			this.fromOffsetRawData(v, 2, header, 4);
		});
	}
	...
	public fromOffsetRawData_invalidVersion_throws() {
		...
		Testers.testUint((v: number) => {
			this.fromOffsetRawData(data, v, header, 4);
		});
	}
	...
	public fromOffsetRawData_falsyHeader_throws() {
		...
		// Act/Assert
		Testers.testTruthy((v: Id3v2FrameHeader) => {
			this.fromOffsetRawData(data, 2, v, 4);
		});
	}
