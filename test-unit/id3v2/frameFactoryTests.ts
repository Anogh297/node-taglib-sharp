...
class FrameFactoryTests {
	...
	public createFrame_invalidVersion() {
		...
		// Act / Assert
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, 0, -1, false);
		});
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, 0, 1.23, false);
		});
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, 0, 0x100, false);
		});
	}
	...
	public createFrame_invalidOffset() {
		...
		// Act / Assert
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, -1, 4, false);
		});
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, 1.23, 4, false);
		});
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, Number.MAX_SAFE_INTEGER + 1, 4, false);
		});
	}
	...
	public createFrame_missingSource() {
		// Act / Assert
		assert.throws(() => {
			FrameFactory.createFrame(undefined, undefined, 0, 4, false);
		});
		assert.throws(() => {
			FrameFactory.createFrame(null, null, 0, 4, false);
		});
	}
	...
	public createFrame_compressed() {
		...
		// Act / Assert
		assert.throws(() => {
			FrameFactory.createFrame(data, undefined, 0, 4, false);
		});
	}
	...
	public createFrame_fromData_mcdi() {
		...
		header.frameSize = 9;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromString("12345abcd", StringType.Latin1));
	...
	public createFrame_fromData_unknown() {
		...
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromString("foo", StringType.UTF8));
	...
	public createFrame_fromOffsetData() {
		// Arrange
		const data = ByteVector.concatenate(0x00, 0x00, UniqueFileIdentifierFrame.fromData("foo", ByteVector.empty()).render(4));
	...
	public createFrame_fromData_customFrameCreatorMatch() {
		try {
			...
			mockCreator.setup((c) => c(TypeMoq.It.isAny(), TypeMoq.It.isValue(0), TypeMoq.It.isAny(), TypeMoq.It.isValue(4))).returns(() => frame);
	...
	public createFrame_fromData_customFrameCreatorNoMatch() {
		try {
			...
			const data = frame.render(4);
			const mockCreator = TypeMoq.Mock.ofType<FrameCreator>();
			mockCreator.setup((c) => c(TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny())).returns(() => undefined);
