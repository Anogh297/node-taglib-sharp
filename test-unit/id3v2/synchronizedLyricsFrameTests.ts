...
class Id3v2_SynchronizedLyricsFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromInfo_withoutEncoding() {
		...
		// Assert
		Id3v2_SynchronizedLyricsFrame_ConstructorTests.assertFrame(frame, description, TimestampFormat.Unknown, language, [], Id3v2Settings.defaultEncoding, textType);
	}
	...
	public fromInfo_withEncoding() {
		...
		// Assert
		Id3v2_SynchronizedLyricsFrame_ConstructorTests.assertFrame(frame, description, TimestampFormat.Unknown, language, [], encoding, textType);
	}
	...
	public fromRawData_notEnoughBytes() {
		...
		header.frameSize = 5;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, 0x00, 0x00, 0x00);

		// Act / Assert
		assert.throws(() => {
			SynchronizedLyricsFrame.fromRawData(data, 4);
		});
	}
	...
	public fromRawData_missingDelimiter() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.SYLT);
		header.frameSize = 10;
		const data = ByteVector.concatenate(header.render(4), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05);
