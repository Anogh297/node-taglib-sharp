...
class Id3v2_PopularimeterFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromRawData_noDelimiter() {
		...
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(4), 0x01, 0x02, 0x03);

		// Act / Assert
		assert.throws(() => {
			PopularimeterFrame.fromRawData(data, 4);
		});
	}
	...
	public fromRawData_tooShort() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.POPM);
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(4), 0x01, ByteVector.getTextDelimiter(StringType.Latin1));
	...
	public fromRawData_noPlayCount() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.POPM);
		header.frameSize = 9;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromString("fux", StringType.Latin1), ByteVector.getTextDelimiter(StringType.Latin1), 0x05);
...
class Id3v2_PopularimeterFrame_MethodTests {
	...
	public find_falsyFrames() {
		// Act / Assert
		Testers.testTruthy((v: PopularimeterFrame[]) => {
			PopularimeterFrame.find(v, "fux");
		});
	}
	...
	public find_noMatches() {
		// Arrange
		const frames = [PopularimeterFrame.fromUser("fux")];
	...
	public find_matches() {
		// Arrange
		const frames = [PopularimeterFrame.fromUser("fux"), PopularimeterFrame.fromUser("bux")];
