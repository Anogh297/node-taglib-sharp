...
class Id3v2_TextInformationFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromRawData_emptyFrameByLength_returnsEmptyFrame() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.TCOP);
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(3), StringType.Latin1);
	...
	public fromRawData_emptyFrameByContents_returnsEmptyFrame() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.TCOP);
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(3), StringType.Latin1, 0x0, 0x0);
	...
	public fromRawData_v4NotTxxx_returnsFrameSplitByDelimiter() {
		...
		const data = ByteVector.concatenate(
			...
			0x0,
			0x0, // Extra nulls to trigger null stripping logic
		);
		...
		// Assert
		Id3v2_TextInformationFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.TCOP, ["fux", "bux"], StringType.UTF16BE);
	}
	...
	public fromRawData_txxx_returnsFrameSplitByDelimiter() {
		...
		// Assert
		Id3v2_TextInformationFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.TXXX, ["fux", "bux"], StringType.UTF16BE);
	}
	...
	public fromRawData_v3SplitType_returnsFrameSplitBySlash() {
		...
		header.frameSize = 8;
		const data = ByteVector.concatenate(header.render(3), StringType.Latin1, ByteVector.fromString("fux/bux", StringType.Latin1));
	...
	public fromRawData_v3TconNonStandardSeparatorEnabled_returnsListOfStringsSeparatorsSplit() {
		...
		const data = ByteVector.concatenate(
			header.render(3),
			StringType.UTF16BE,
			ByteVector.fromString("(32)Classical(CR)(RX)Whoa here's some cra((z)y string;here's another/ / one more", StringType.UTF16BE),
		);
		...
		try {
			...
			Id3v2_TextInformationFrame_ConstructorTests.assertFrame(
				...
				["Classical", "Cover", "Remix", "Whoa here's some cra(z)y string", "here's another", "one more"],
				StringType.UTF16BE,
			);
		} finally {
	...
	public fromRawData_v3TconNonStandardNumericGenresEnabled_returnsDecodedGenreList() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.TCON);
		header.frameSize = 13;
		const data = ByteVector.concatenate(header.render(3), StringType.UTF16BE, ByteVector.fromString("(32)12", StringType.UTF16BE));
		...
		try {
			...
			// Assert
			Id3v2_TextInformationFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.TCON, ["Classical", "Other"], StringType.UTF16BE);
		} finally {
			// Cleanup - Restore settings
	...
	public fromRawData_v3TconNonStandardNumericGenresDisabled_returnsNumericValue() {
		...
		try {
			...
			// Assert
			Id3v2_TextInformationFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.TCON, ["Classical", "12"], StringType.UTF16BE);
		} finally {
			// Cleanup - Restore settings
			Id3v2Settings.useNonStandardV2V3NumericGenres = originalNonStandardNumericGenreSetting;
		}
	...
	public fromRawData_v4Tcon_returnsListOfStrings() {
		...
		// Assert
		Id3v2_TextInformationFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.TCON, ["Classical", "(32)", "some genre"], StringType.UTF16BE);
	}
	...
	private static assertFrame(frame: TextInformationFrame, frameId: FrameIdentifier, text: string[], encoding: StringType = StringType.Latin1): void {
		assert.isOk(frame);
		assert.strictEqual(frame.frameClassType, FrameClassType.TextInformationFrame);
...
class Id3v2_TextInformationFrame_MethodTests {
	...
	public find_invalidIdentity() {
		// Act
		Testers.testTruthy((v: FrameIdentifier) => {
			TextInformationFrame.findTextInformationFrame([], v);
		});
	}
	...
	public find_frameExists() {
		// Arrange
		const frames = [TextInformationFrame.fromIdentifier(FrameIdentifiers.TCOP), TextInformationFrame.fromIdentifier(FrameIdentifiers.TCOM)];
	...
	public render_readV3IsTxxxEmpty() {
		...
		header.frameSize = 1;
		const data = ByteVector.concatenate(header.render(3), StringType.UTF8);
		const frame = TextInformationFrame.fromRawData(data, 3);
		const _ = frame.text; // Force a read
		...
		header.frameSize = 3;
		const expected = ByteVector.concatenate(header.render(3), StringType.UTF16, ByteVector.getTextDelimiter(StringType.UTF16));
	...
	public render_readV3IsTxxxSingleValue() {
		...
		let header = new Id3v2FrameHeader(FrameIdentifiers.TXXX);
		header.frameSize = 7;
		const data = ByteVector.concatenate(header.render(3), StringType.UTF16BE, ByteVector.fromString("fux", StringType.UTF16BE));
		const frame = TextInformationFrame.fromRawData(data, 3);
		const _ = frame.text; // Force a read
	...
	public render_readV3Tcon_numericGenresEnabled() {
		...
		try {
			...
			const data = ByteVector.concatenate(
				header.render(3),
				StringType.UTF16BE,
				ByteVector.fromString("(32)Classical(CR)(32)(RX)SomeGenre;Whoa here's some cra((z)y string", StringType.UTF16BE),
			);
			const frame = TextInformationFrame.fromRawData(data, 3);
			const _ = frame.text; // Force a read
	...
	public render_readV3Tcon_numericGenresDisabled() {
		...
		try {
			...
			const expected = ByteVector.concatenate(
				header.render(3),
				StringType.UTF16BE,
				ByteVector.fromString("(CR)(RX)Classical;Classical;SomeGenre;Whoa here's some cra((z)y string", StringType.UTF16BE),
			);
	...
	public render_readV3WithSplit() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.TCOP);
		header.frameSize = 15;
		const data = ByteVector.concatenate(header.render(3), StringType.UTF16BE, ByteVector.fromString("fux/bux", StringType.UTF16BE));
		const frame = TextInformationFrame.fromRawData(data, 3);
		const _ = frame.text; // Force a read
