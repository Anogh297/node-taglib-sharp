...
class Id3v2_UnknownFrame_ConstructorTests extends FrameConstructorTests {
	...
	public fromData_falsyType_throws() {
		...
		Testers.testTruthy((v: FrameIdentifier) => {
			UnknownFrame.fromData(v, undefined);
		});
	}
	...
	public fromOffsetData_validParams_returnsFrame() {
		...
		header.frameSize = 11;
		const data = ByteVector.concatenate(header.render(4), 0x0, 0x0, ByteVector.fromString("foo bar baz", StringType.UTF8));

		// Act
		const frame = UnknownFrame.fromOffsetRawData(data, 2, header, 4);

		// Assert
		Id3v2_UnknownFrame_ConstructorTests.assertFrame(frame, FrameIdentifiers.WXXX, ByteVector.fromString("foo bar baz", StringType.UTF8));
	}
	...
	public fromRawData_validParams_returnsFrame() {
		...
		const header = new Id3v2FrameHeader(FrameIdentifiers.WXXX);
		header.frameSize = 11;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromString("foo bar baz", StringType.UTF8));
