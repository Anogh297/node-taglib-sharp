...
class Id3v2_UserInformationFrame_ConstructorTests extends FrameConstructorTests {
	...
	private static assertFrame(frame: UserTextInformationFrame, description: string, text: string[], encoding: StringType) {
		assert.isOk(frame);
...
class Id3v2_UserTextInformationFrame_MethodTests {
	...
	public findUserTextInformationFrame_frameWithCaseSensitiveDescriptionDoesNotExist() {
		// Arrange
		const frames = [UserTextInformationFrame.fromDescription("fUX"), UserTextInformationFrame.fromDescription("bUX")];
