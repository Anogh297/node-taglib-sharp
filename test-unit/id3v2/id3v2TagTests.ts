...
const getTestTagHeader = (version: number, flags: Id3v2TagHeaderFlags, tagSize: number): ByteVector => {
	return ByteVector.concatenate(ByteVector.fromString("ID3", StringType.Latin1), version, 0x00, flags, SyncData.fromUint(tagSize));
};
...
class Id3v2_Tag_ConstructorTests {
	...
	public fromData_dataTooShortForTagLength() {
		// Arrange
		const data = ByteVector.concatenate(getTestTagHeader(4, Id3v2TagHeaderFlags.None, 5), 0x00, 0x00);
	...
	public fromData_v4Tag() {
		...
		const frame2 = UniqueFileIdentifierFrame.fromData("foo", ByteVector.fromString("bar", StringType.UTF8)).render(4);
	...
	public fromData_needsResync() {
		// Arrange
		const frame1 = PlayCountFrame.fromEmpty().render(4);
		const data = ByteVector.concatenate(getTestTagHeader(3, Id3v2TagHeaderFlags.Unsynchronization, frame1.length), frame1);
	...
	public fromFileStart_positionTooFar() {
		...
		// Act / Assert
		assert.throws(() => {
			Id3v2Tag.fromFileStart(mockFile.object, 5, ReadStyle.None);
		});
	}
...
class Id3v2_Tag_PropertyTests {
	...
	public replayGainTrackGain() {
		// Arrange
		const tag = Id3v2Tag.fromEmpty();
		const setProp = (v: number) => {
			tag.replayGainTrackGain = v;
		};
	...
	public pictures() {
		...
		const pictures = [mockPicture1.object, mockPicture2.object];
	...
	private testArrayTextFrameProperty(set: (t: Id3v2Tag, v: string[]) => void, get: (t: Id3v2Tag) => string[], fId: FrameIdentifier) {
		// Arrange
		const tag = Id3v2Tag.fromEmpty();
	...
	private testTextFrameProperty(set: (t: Id3v2Tag, v: string) => void, get: (t: Id3v2Tag) => string, fId: FrameIdentifier) {
		// Arrange
		const tag = Id3v2Tag.fromEmpty();
		const setProp = (v: string) => set(tag, v);
		const getProp = () => get(tag);
	...
	private testUserTextFrameProperty(set: (t: Id3v2Tag, v: string) => void, get: (t: Id3v2Tag) => string, desc: string) {
		// Arrange
		const tag = Id3v2Tag.fromEmpty();
		const setProp = (v: string) => set(tag, v);
		const getProp = () => get(tag);
...
class Id3v2_Tag_MethodTests {
	...
	public copyTo_invalidDestination() {
		...
		// Act / Assert
		assert.throws(() => {
			source.copyTo(undefined, true);
		});
		assert.throws(() => {
			source.copyTo(null, true);
		});
	...
	public copyTo_noOverwrite() {
		...
		const dPcnt = dest.getFramesByIdentifier<PlayCountFrame>(FrameClassType.PlayCountFrame, FrameIdentifiers.PCNT)[0];
	...
	public copyTo_overwrite() {
		...
		const dTcom = dest.getFramesByIdentifier<TextInformationFrame>(FrameClassType.TextInformationFrame, FrameIdentifiers.TCOM)[0];
	...
	public getFramesByClassType_invalidClassType() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.getFramesByClassType<TextInformationFrame>(undefined);
		});
		assert.throws(() => {
			tag.getFramesByClassType<TextInformationFrame>(null);
		});
	}
	...
	public getFramesByIdentifier_invalidClassType() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.getFramesByIdentifier<TextInformationFrame>(undefined, FrameIdentifiers.TCOM);
		});
		assert.throws(() => {
			tag.getFramesByIdentifier<TextInformationFrame>(null, FrameIdentifiers.TCOM);
		});
	}
	...
	public getFramesByIdentifier_invalidIdentifier() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.getFramesByIdentifier<TextInformationFrame>(classType, undefined);
		});
		assert.throws(() => {
			tag.getFramesByIdentifier<TextInformationFrame>(classType, null);
		});
	}
	...
	public getFramesByIdentifier_hasMatches() {
		...
		// Act
		const result = tag.getFramesByIdentifier<TextInformationFrame>(FrameClassType.TextInformationFrame, FrameIdentifiers.TCOM);
	...
	public getTextAsString_invalidIdentity() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.getTextAsString(undefined);
		});
		assert.throws(() => {
			tag.getTextAsString(null);
		});
	}
	...
	public render_v4_noFooter() {
		...
		const expected = ByteVector.concatenate(header.render(), frame1Data, frame2Data, ByteVector.fromSize(1024, 0x00));
	...
	public render_v4_hasFooter() {
		...
		const expected = ByteVector.concatenate(header.render(), frame1Data, frame2Data, Id3v2TagFooter.fromHeader(header).render());
	...
	public render_v3_unsyncAtTagLevel() {
		...
		// Assert
		let frameData = ByteVector.concatenate(frame1.render(3), frame2.render(3));
		...
		const expected = ByteVector.concatenate(header.render(), frameData, ByteVector.fromSize(1024, 0x00));
	...
	public replaceFrame_invalidFrames() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.replaceFrame(undefined, frame);
		});
		assert.throws(() => {
			tag.replaceFrame(null, frame);
		});
		assert.throws(() => {
			tag.replaceFrame(frame, undefined);
		});
		assert.throws(() => {
			tag.replaceFrame(frame, null);
		});
	}
	...
	public setNumberFrame_invalidArgs() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.setNumberFrame(undefined, 1, 1, 1);
		});
		assert.throws(() => {
			tag.setNumberFrame(null, 1, 1, 1);
		});

		assert.throws(() => {
			tag.setNumberFrame(identifier, -1, 1, 1);
		});
		assert.throws(() => {
			tag.setNumberFrame(identifier, 1.23, 1, 1);
		});
		assert.throws(() => {
			tag.setNumberFrame(identifier, Number.MAX_SAFE_INTEGER + 1, 1, 1);
		});
		...
		assert.throws(() => {
			tag.setNumberFrame(identifier, 1, 1.23, 1);
		});
		assert.throws(() => {
			tag.setNumberFrame(identifier, 1, Number.MAX_SAFE_INTEGER + 1, 1);
		});
		...
		assert.throws(() => {
			tag.setNumberFrame(identifier, 1, 1, 1.23);
		});
		assert.throws(() => {
			tag.setNumberFrame(identifier, 1, 1, 0x100);
		});
	}
	...
	public setTextFrame_invalidArgs() {
		...
		// Act / Assert
		assert.throws(() => {
			tag.setTextFrame(undefined, "foo");
		});
		assert.throws(() => {
			tag.setTextFrame(null, "foo");
		});
	}
