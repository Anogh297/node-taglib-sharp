...
class TestFrame extends Frame {
	public static renderFieldData = ByteVector.concatenate(ByteVector.fromSize(10, 0x00), 0xff, 0xe0, ByteVector.fromSize(10, 0x00));
...
class FrameTests {
	...
	public render_dataLengthIndicator() {
		...
		// Assert
		const expected = ByteVector.concatenate(header.render(4), ByteVector.fromUint(TestFrame.renderFieldData.length), TestFrame.renderFieldData);
	...
	public render_groupingIdentity() {
		...
		// Assert
		const expected = ByteVector.concatenate(header.render(4), 0x88, TestFrame.renderFieldData);
		Testers.bvEqual(output, expected);
	}
	...
	public fieldData_dataLengthIndicator() {
		...
		const frame = new TestFrame(header);
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromUint(TestFrame.renderFieldData.length), TestFrame.renderFieldData);
	...
	public fieldData_groupingIdentify() {
		...
		const frame = new TestFrame(header);
		const data = ByteVector.concatenate(header.render(4), 0x88, TestFrame.renderFieldData);
	...
	public fieldData_desynchronized() {
		...
		const frame = new TestFrame(header);
		const data = ByteVector.concatenate(header.render(4), fieldData);
	...
	public fieldData_noHeaderInData() {
		// Arrange
		const fieldData = ByteVector.concatenate(0x00, 0x00, TestFrame.renderFieldData);
