...
class Id3v2_RelativeVolumeChannelData {
	...
	public peakVolume_setOutsideBitRange() {
		...
		// Act / Assert
		assert.throws(() => {
			channel.peakVolume = BigInt(0x100);
		});
		assert.throws(() => {
			channel.peakVolume = BigInt(-100);
		});
	}
	...
	public volumeAdjustment_invalidValues() {
		...
		Testers.testInt((v: number) => {
			channel.volumeAdjustment = v;
		});
		assert.throws(() => {
			channel.volumeAdjustment = -64;
		});
		assert.throws(() => {
			channel.volumeAdjustment = 64;
		});
	...
	public render_lessThan8BitsStoresAsByte() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x07, 0x1f);
	...
	public render_15BitsStoresAsShort() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x0f, 0x1f, 0xff);
		Testers.bvEqual(output, expected);
	}
	...
	public render_23BitsStoresAs3Bytes() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x17, 0x1f, 0xff, 0xff);
		Testers.bvEqual(output, expected);
	}
	...
	public render_31BitsStoresAs4Bytes() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x1f, 0x1f, 0xff, 0xff, 0xff);
		Testers.bvEqual(output, expected);
	}
	...
	public render_39BitsStoresAs5Bytes() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x27, 0x1f, 0xff, 0xff, 0xff, 0xff);
		Testers.bvEqual(output, expected);
	}
	...
	public render_64BitsStoresAs8Bytes() {
		...
		// Assert
		const expected = ByteVector.concatenate(ChannelType.Subwoofer, 0x40, 0x00, 0x40, 0x1f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
		Testers.bvEqual(output, expected);
	}
}
...
class Id3v2_RelativeVolumeFrame_ConstructorTests extends ConstructorTests {
	...
	public fromRawData_noDelimiter_emptyFrame() {
		...
		header.frameSize = 3;
		const data = ByteVector.concatenate(header.render(4), ByteVector.fromString("foo", StringType.Latin1));
...
class Id3v2_RelativeVolumeFrameMethodTests {
	...
	public find_falsyFrames() {
		// Act / Assert
		Testers.testTruthy((v: RelativeVolumeFrame[]) => {
			RelativeVolumeFrame.find(v, "foo");
		});
	}
	...
	public find_noMatches() {
		// Arrange
		const frames = [RelativeVolumeFrame.fromIdentification("fux"), RelativeVolumeFrame.fromIdentification("bux")];
	...
	public peakBits() {
		...
		PropertyTests.propertyRoundTrip(
			(v) => {
				frame.setPeakBits(ChannelType.Subwoofer, v);
			},
	...
	public peakVolume() {
		...
		PropertyTests.propertyRoundTrip(
			(v) => {
				frame.setPeakVolume(ChannelType.Subwoofer, v);
			},
	...
	public volumeAdjustment() {
		...
		PropertyTests.propertyRoundTrip(
			(v) => {
				frame.setVolumeAdjustment(ChannelType.Subwoofer, v);
			},
