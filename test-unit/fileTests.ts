...
class FileTests {
	...
	public createFromAbstraction_noMimetypeProvided() {
		...
		testResolver.setup((r) => r(TypeMoq.It.isAny(), TypeMoq.It.isAnyString(), TypeMoq.It.isAny())).returns(() => mockFile);
		...
		try {
			...
			testResolver.verify((r) => r(TypeMoq.It.isAny(), TypeMoq.It.isValue("taglib/qux"), TypeMoq.It.isValue(ReadStyle.Average)), TypeMoq.Times.once());
		} finally {
			// Cleanup
	...
	public createFromPath_mimetypeNotProvided() {
		// Arrange
		const testResolver = TypeMoq.Mock.ofType<FileTypeResolver>();
		testResolver.setup((r) => r(TypeMoq.It.isAny(), TypeMoq.It.isAnyString(), TypeMoq.It.isAny())).returns((a) => new TestFile(a));
		File.addFileTypeResolver(testResolver.object);
		...
		try {
			...
			testResolver.verify((r) => r(TypeMoq.It.isAny(), TypeMoq.It.isValue("taglib/txt"), TypeMoq.It.isValue(ReadStyle.Average)), TypeMoq.Times.once());
		} finally {
			// Cleanup
			File.removeFileTypeResolver(testResolver.object);
		}
	...
	public createInternal_unsupported() {
		// Arrange
		const testResolver = TypeMoq.Mock.ofType<FileTypeResolver>();
		testResolver.setup((r) => r(TypeMoq.It.isAny(), TypeMoq.It.isAnyString(), TypeMoq.It.isAny())).returns(() => undefined);
		File.addFileTypeResolver(testResolver.object);
	...
	public addFileType_invalidParameters() {
		// Act / Assert
		Testers.testString((v: string) => {
			File.addFileType(v, TestFile, false);
		});
		Testers.testTruthy((v: FileTypeConstructor) => {
			File.addFileType("foo/bar", v, false);
		});
	}
	...
	public addFileTypeResolver_invalidParameters() {
		// Act / Assert
		Testers.testTruthy((v: FileTypeResolver) => {
			File.addFileTypeResolver(v);
		});
	}
	...
	public find_file1() {
		const testAction = (f: TestFile) => {
			...
			// Act / Assert
			assert.strictEqual(f.find(ByteVector.fromString("U", StringType.Latin1)), FileTests.chars.indexOf("U"));
	...
	public find_file2() {
		const testAction = (f: TestFile) => {
			// Act / Assert
			assert.strictEqual(f.find(ByteVector.fromString("M", StringType.Latin1)), FileTests.chars.indexOf("M"));
	...
	public rFind_file1() {
		const testAction = (f: TestFile, d: ByteVector) => {
			// Act / Assert
			assert.strictEqual(f.rFind(ByteVector.fromString("U", StringType.Latin1)), d["_bytes"].lastIndexOf("U".charCodeAt(0)));
	...
	public rFind_file2() {
		const testAction = (f: TestFile, d: ByteVector) => {
			// Act / Assert
			assert.strictEqual(f.rFind(ByteVector.fromString("M", StringType.Latin1)), d["_bytes"].lastIndexOf("M".charCodeAt(0)));
