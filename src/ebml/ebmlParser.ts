...
export default class EbmlParser implements IDisposable {
	...
	public static getAllElements(parser: EbmlParser): Map<number, EbmlElement> {
		try {
			...
			while (parser.read()) {
				elements.set(parser.currentElement.id, parser.currentElement);
			}
	...
	public setOptions(maxIdLength: number | undefined, maxSizeLength: number | undefined): void {
		...
		if (maxIdLength > 8 || maxSizeLength > 8) {
			throw new UnsupportedFormatError("Not supported: This EBML file is not supported in this version of node-taglib-sharp.");
		}
	...
	private static lastSevenBitsTruthy(bytes: ByteVector, index: number, upperMask: number, lowerMask: number): boolean {
		const upperByteCheck = index > 1 ? NumberUtils.hasFlag(bytes.get(index - 1), upperMask) : false;
	...
	private renderVariableInteger(value: number | bigint): ByteVector {
		...
		while (lowerByteIndex < 8) {
			if (EbmlParser.lastSevenBitsTruthy(valueBytes, lowerByteIndex, upperMask, lowerMask)) {
				...
				return ByteVector.concatenate(NumberUtils.uintOr(lengthDescriptor, upperMostByte), valueBytes.subarray(lowerByteIndex));
			}
	...
	private readVariableInteger(maxBytes: number): { bytes: number; value: number } {
		...
		for (let i = 0; i < additionalBytes; i++) {
			...
			if (additionalBytes >= 7 && i >= 6 && NumberUtils.hasFlag(byte, 0xf0)) {
				throw new Error("Not supported: EBML data sizes > 52 bits are not supported in this version of node-taglib-sharp");
			}
