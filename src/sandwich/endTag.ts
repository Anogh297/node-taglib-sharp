...
export default class EndTag extends CombinedTag {
	...
	public render(): ByteVector {
		...
		const tagBytes = this.tags.sort((t1, t2) => t2.tagTypes - t1.tagTypes).map((t) => (<ApeTag | Id3v1Tag | Id3v2Tag>t).render());
	...
	private read(file: File, style: ReadStyle): void {
		...
		tags.sort((t1, t2) => t2.tagTypes - t1.tagTypes).forEach((t) => this.addTag(t));
	}
}
...
class EndTagParser extends TagParser {
	...
	private static readonly READ_SIZE = Math.max(ApeTagFooter.SIZE, Id3v2Settings.footerSize, Id3v1Tag.TOTAL_SIZE);
