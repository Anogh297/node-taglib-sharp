...
export class FlacBlock implements ILazy {
	...
	private constructor() {
		/* private to enforce creation via static constructors */
	}
	...
	public get totalSize(): number {
		return this.dataSize + FlacBlock.HEADER_SIZE;
	}

	/**
	 * Gets the type of data contained in the current instance.
	 */
	public get type(): FlacBlockType {
		return this._type;
	}
	...
	public render(isLastBlock: boolean): ByteVector {
		...
		const header = NumberUtils.uintOr(this._dataSize, NumberUtils.uintLShift(NumberUtils.uintOr(this._type, isLastBlock ? 0x80 : 0), 24));

		return ByteVector.concatenate(ByteVector.fromUint(header), this._data);
	}
