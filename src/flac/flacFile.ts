...
export default class FlacFile extends File implements ISandwichFile {
	...
	public get mediaEndPosition(): number {
		return this._mediaEndPosition;
	}

	public get mediaStartPosition(): number {
	...
	private readXiphComments(readStyle: ReadStyle): XiphComment {
		...
		return xiphComments.reduce((accum, xc) => {
...
["taglib/flac", "audio/x-flac", "audio/flc", "application/x-flac"].forEach((mt) => File.addFileType(mt, FlacFile));
