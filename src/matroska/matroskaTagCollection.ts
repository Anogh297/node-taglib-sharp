...
export default class MatroskaTagCollection extends Tag {
	...
	public constructor(sizeOnDisk: number, isVideo: boolean, tags: MatroskaTag[], attachments: MatroskaAttachment[]) {
		...
		this._albumTagLevel = this._isVideo ? MatroskaTagTargetType.COLLECTION.value : MatroskaTagTargetType.ALBUM.value;
		this._fileTagLevel = this._isVideo ? MatroskaTagTargetType.EPISODE.value : MatroskaTagTargetType.TRACK.value;
		this._albumPartTagLevel = this._fileTagLevel + 10;
		this._performerKey = this._isVideo ? MatroskaTagCollection.PERFORMER_VIDEO_KEY : MatroskaTagCollection.PERFORMER_AUDIO_KEY;
	 ...
	 */
	public get attachments(): MatroskaAttachment[] {
	...
	public get titleSort(): string {
		return this.getFileTagValues("TITLE").getSubtags("SORT_WITH").firstString;
	}
	...
	public get performersSort(): string[] {
		return this.getTagValuesRecursively(this._fileTagLevel, this._performerKey).getSubtags("SORT_WITH").allStrings;
	}

	/** @inheritDoc */
	public get performersRole(): string[] {
		const roleSubkey = this._isVideo ? "CHARACTER" : "INSTRUMENTS";
		return this.getTagValuesRecursively(this._fileTagLevel, this._performerKey).getSubtags(roleSubkey).allStrings;
	}
	...
	public get albumArtistsSort(): string[] {
		return this.getTagValues(this._albumTagLevel, "ARTIST").getSubtags("SORT_WITH").allStrings;
	}
	...
	public get composersSort(): string[] {
		return this.getTagValues(this._fileTagLevel, "COMPOSER").getSubtags("SORT_WITH").allStrings;
	}
	...
	public get albumSort(): string {
		return this.getTagValues(this._albumTagLevel, "TITLE").getSubtags("SORT_WITH").firstString;
	}
	...
	public get genres(): string[] {
		...
		for (let i = 0; i < result.length; i++) {
			...
			const numericGenre = this._isVideo ? Genres.indexToVideo(trimmedGenre, true) : Genres.indexToAudio(trimmedGenre, true);
	...
	public get isEmpty(): boolean {
		return this._attachments.length === 0 && this._tags.length === 0;
	}
	...
	private getTagValuesRecursively(level: number, key: string): FilteredTags {
		...
		return tagsAtLevel.length === 0 ? this.getTagValuesRecursively(level + 10, key) : tagsAtLevel;
	}
	...
	private getTagValues(level: number | undefined, key: string): FilteredTags {
		...
		const targetTypeValueFunc = level ? (t: MatroskaTagTarget) => t.targetType && t.targetType.value === level : (t: MatroskaTagTarget) => !t.targetType;
...
class FilteredTags {
	...
	public get allStrings(): string[] {
		return this._tags.filter((t) => t.isString).map((t) => <string>t.value);
	}

	public get firstString(): string {
		const firstStringTag = this._tags.find((t) => t.isString);
		return firstStringTag === undefined ? undefined : <string>firstStringTag.value;
	}
	...
	public getSubtags(subkey: string): FilteredTags {
		...
		const filteredSubtags = this._tags.reduce<MatroskaTagValue[]>((accum, tag) => accum.concat(tag.nestedTags), []).filter((st) => st.name === subkey);
