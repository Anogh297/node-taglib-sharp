...
export default class MatroskaFile extends File {
	...
	private readEbmlHeader(headerElement: EbmlElement): void {
		...
		if (MatroskaFile.SUPPORTED_DOCTYPES.indexOf(result.docType) < 0) {
			throw new UnsupportedFormatError(`EBML doctype ${result.docType} is not supported by Matroska file loader`);
		}
	...
	private readTag(tagElement: EbmlElement): MatroskaTag[] {
		...
		const parserActions = new Map<number, (e: EbmlElement) => void>([
			[MatroskaIds.SIMPLE_TAG, (e) => simpleTags.push(MatroskaTagValue.fromSimpleTagElement(e, this._header.docTypeVersion))],
			[
				MatroskaIds.TARGETS,
				(e) => {
	...
	private readTags(tagsElement: EbmlElement, readState: TagReadState): void {
		...
		const parserActions = new Map<number, (e: EbmlElement) => void>([[MatroskaIds.TAG, (e) => tagCollections.push(this.readTag(e))]]);
	...
	private readTracks(tracksElement: EbmlElement): void {
		...
		const trackParseActions = new Map<number, (e: EbmlElement) => void>([[MatroskaIds.TRACK_ENTRY, (e) => this._tracks.push(TrackFactory.fromTrackElement(e))]]);
...
["taglib/mk3d", "taglib/mka", "taglib/mks", "taglib/mkv", "taglib/webm", "audio/webm", "audio/x-matroska", "video/webm", "video/x-matroska"].forEach((mt) =>
	File.addFileType(mt, MatroskaFile),
);
