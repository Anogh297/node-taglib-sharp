...
export default class AiffStreamHeader implements ILosslessAudioCodec {
	...
	public get audioBitrate(): number {
		return this.durationMilliseconds <= 0 ? 0 : (this._streamLength * 8) / (this.durationMilliseconds / 1000) / 1000;
	}
	...
	public get durationMilliseconds(): number {
		return this._sampleRate <= 0 || this._totalFrames <= 0 ? 0 : (this._totalFrames / this._sampleRate) * 1000;
	}
