...
export default class AppleItemListBox extends Mpeg4Box {
	...
	public setItunesTagBoxes(meanString: string, nameString: string, dataStrings: string[]): void {
		...
		for (const dataString of dataStrings) {
			...
			const meanBox = AppleAdditionalInfoBox.fromTypeVersionAndFlags(Mpeg4BoxType.MEAN, 0, AppleDataBoxFlagType.ContainsText);
			meanBox.text = meanString;

			const nameBox = AppleAdditionalInfoBox.fromTypeVersionAndFlags(Mpeg4BoxType.NAME, 0, AppleDataBoxFlagType.ContainsText);
