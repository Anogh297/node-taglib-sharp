...
export class Properties implements ILosslessAudioCodec, IVideoCodec, IPhotoCodec {
	 ...
	 */
	public get codecs(): ICodec[] {
		return this._codecs.slice();
	}
	...
	public get durationMilliseconds(): number {
		...
		return this._codecs.filter((e) => !!e).reduce((maxDuration, e) => Math.max(maxDuration, e.durationMilliseconds), 0);
	}
	...
	private findCodecProperty<TCodec extends ICodec>(mediaType: MediaTypes, property: (codec: TCodec) => number, defaultValue: number): number {
		const codec = this._codecs.find((e) => !!e && NumberUtils.hasFlag(e.mediaTypes, mediaType));
		return codec ? property(<TCodec>codec) || defaultValue : defaultValue;
	}
