...
export default class UuidWrapper {
	private static readonly GUID_REGEX = new RegExp(/([\dA-F]{8})-?([\dA-F]{4})-?([\dA-F]{4})-?([\dA-F]{4})-?([\dA-F]{12})/i);
	...
	public equals(b: UuidWrapper): boolean {
		if (!b || this._bytes.length !== b._bytes.length) {
			return false;
		}
	 ...
	 */
	public toBytes(): ByteVector {
		return this._bytes.toByteVector();
	}
