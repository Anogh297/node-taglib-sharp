...
export default class MpegContainerFile extends SandwichFile {
	...
	public constructor(file: IFileAbstraction | string, propertiesStyle: ReadStyle) {
		super(file, propertiesStyle, MpegContainerFile.DEFAULT_TAG_LOCATION_MAPPING, MpegContainerFileSettings.defaultTagTypes);
	}
	...
	protected readProperties(readStyle: ReadStyle): Properties {
		...
		const codecs = [];
		if (this._videoHeader) {
			codecs.push(this._videoHeader);
		}
		if (this._audioHeader) {
			codecs.push(this._audioHeader);
		}

		// @TODO: Can we omit calculating duration via start/end timestamps if we have audio with
		//     a valid duration?
		// Calculate duration of the file
		const lastSyncPosition = this.rFindMarkerPosition(this.length - this.mediaEndPosition, MpegFileMarker.SystemSyncPacket);
		this._endTime = this.readTimestamp(lastSyncPosition + 4);
		const durationMilliseconds =
			this._startTime === undefined ? (this._audioHeader ? this._audioHeader.durationMilliseconds : 0) : (this._endTime - this._startTime) * 1000;
	...
	private findNextMarkerPosition(position: number, marker: MpegFileMarker): number {
		const packet = ByteVector.concatenate(MpegContainerFile.MARKER_START, marker);
	...
	private readAudioPacket(position: number): number {
		...
		const dataOffset =
			4 +
			2 +
			i + // Packet marker + packet length + stuffing bytes/STD buffer size
	...
	private readSystemFile(position: number): void {
		...
		for (let i = 0; i < sanityLimit && (this._startTime === undefined || !this._audioFound || !this._videoFound); i++) {
...
["taglib/mpg", "taglib/mpeg", "taglib/mpe", "taglib/mpv2", "taglib/m2v", "video/x-mpg", "video/mpeg"].forEach((mt) => File.addFileType(mt, MpegContainerFile));
