...
export default class MpegAudioHeader implements IAudioCodec {
	...
	private constructor() {
		/* private to enforce construction via static methods */
	}
	 ...
	 */
	public static fromInfo(flags: number, streamLength: number, xingHeader: XingHeader, vbriHeader: VbriHeader): MpegAudioHeader {
	...
	public get audioBitrate(): number {
		...
		if (this._xingHeader.totalSize > 0 && this._xingHeader.totalFrames > 0 && this.durationMilliseconds / 1000 > 0) {
			return Math.round((this._xingHeader.totalSize * 8) / (this.durationMilliseconds / 1000) / 1000);
		}
		if (this._vbriHeader.totalSize > 0 && this._vbriHeader.totalFrames > 0 && this.durationMilliseconds / 1000 > 0) {
	...
	public get audioChannels(): number {
		return this.channelMode === ChannelMode.SingleChannel ? 1 : 2;
	}
	...
	public get isOriginal(): boolean {
		return ((this._flags >> 2) & 1) === 1;
	}

	/**
	 * Whether or not the audio represented by the current instance is padded.
	 */
	public get isPadded(): boolean {
		return ((this._flags >> 9) & 1) === 1;
	}
	 ...
	 * was found.
	 */
	public get vbriHeader(): VbriHeader {
	 ...
	 * was found.
	 */
	public get xingHeader(): XingHeader {
