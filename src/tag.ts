...
export abstract class Tag {
	 ...
	 */
	public get firstAlbumArtist(): string {
		return Tag.firstInGroup(this.albumArtists);
	}

	/**
	 * Gets the first value contained in {@link albumArtistsSort}
	 */
	public get firstAlbumArtistSort(): string {
		return Tag.firstInGroup(this.albumArtists);
	}
	 ...
	 */
	public get firstPerformer(): string {
		return Tag.firstInGroup(this.performers);
	}

	/**
	 * Gets the first value contained in {@link performersSort}
	 */
	public get firstPerformerSort(): string {
		return Tag.firstInGroup(this.performersSort);
	}

	/**
	 * Gets the first value contained in {@link composers}
	 */
	public get firstComposer(): string {
		return Tag.firstInGroup(this.composers);
	}

	/**
	 * Gets the first value contained in {@link composersSort}
	 */
	public get firstComposerSort(): string {
		return Tag.firstInGroup(this.composersSort);
	}

	/**
	 * Gets the first value contained in {@link genres}
	 */
	public get firstGenre(): string {
		return Tag.firstInGroup(this.genres);
	}

	/**
	 * Gets a semicolon and space separated string containing the values in {@link albumArtists}
	 */
	public get joinedAlbumArtists(): string {
		return Tag.joinGroup(this.albumArtists);
	}

	/**
	 * Gets a semicolon and space separated string containing the values in {@link performers}
	 */
	public get joinedPerformers(): string {
		return Tag.joinGroup(this.performers);
	}

	/**
	 * Gets a semicolon and space separated string containing the values in {@link performersSort}
	 */
	public get joinedPerformersSort(): string {
		return Tag.joinGroup(this.performersSort);
	}

	/**
	 * Gets a semicolon and space separated string containing the values in {@link composers}
	 */
	public get joinedComposers(): string {
		return Tag.joinGroup(this.composers);
	}

	/**
	 * Gets a semicolon and space separated string containing the values in {@link genres}
	 */
	public get joinedGenres(): string {
		return Tag.joinGroup(this.genres);
	}
	...
	public get isEmpty(): boolean {
		return (
			Tag.isFalsyOrLikeEmpty(this.title) &&
	...
	public copyTo(target: Tag, overwrite: boolean): void {
		...
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.title)) {
			target.title = this.title;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.subtitle)) {
			target.subtitle = this.subtitle;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.description)) {
			target.description = this.description;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.albumArtists)) {
			target.albumArtists = this.albumArtists;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.performers)) {
			target.performers = this.performers;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.performersRole)) {
			target.performersRole = this.performersRole;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.composers)) {
			target.composers = this.composers;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.album)) {
			target.album = this.album;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.comment)) {
			target.comment = this.comment;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.genres)) {
		...
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.initialKey)) {
			target.initialKey = this.initialKey;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.publisher)) {
			target.publisher = this.publisher;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.isrc)) {
			target.isrc = this.isrc;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.remixedBy)) {
			target.remixedBy = this.remixedBy;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.grouping)) {
			target.grouping = this.grouping;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.conductor)) {
			target.conductor = this.conductor;
		}
		if (overwrite || Tag.isFalsyOrLikeEmpty(target.copyright)) {
			target.copyright = this.copyright;
		}
		if (overwrite || !target.dateTagged) {
			target.dateTagged = this.dateTagged;
		}
	}
	...
	protected static firstInGroup(group: string[]): string {
		return !group || group.length === 0 ? undefined : group[0];
	}
	...
	protected static joinGroup(group: string[]): string {
		return !group || group.length === 0 ? undefined : group.join("; ");
	}
}
