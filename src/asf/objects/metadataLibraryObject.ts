...
export class MetadataDescriptor extends DescriptorBase {
	 ...
	 */
	public constructor(languageListIndex: number, streamNumber: number, name: string, type: DataType, value: DescriptorValue) {
	 ...
	 */
	public get languageListIndex(): number {
		return this._languageListIndex;
	}

	/**
	 * Gets the index of the stream associated with the current instance.
	 */
	public get streamNumber(): number {
...
export class MetadataLibraryObject extends BaseObject {
	...
	public get objectType(): ObjectType {
		return ObjectType.MetadataLibraryObject;
	}

	/**
	 * Gets all records stored in the current instance.
	 */
	public get records(): MetadataDescriptor[] {
	...
	public getRecords(languageListIndex: number, streamNumber: number, ...names: string[]): MetadataDescriptor[] {
		...
		return this._records.filter((r) => r.languageListIndex === languageListIndex && r.streamNumber === streamNumber && names.indexOf(r.name) >= 0);
	}
	...
	/** @inheritDoc */
	public render(): ByteVector {
		const output = ByteVector.concatenate(ReadWriteUtils.renderWord(this._records.length), ...this._records.map((r) => r.render()));
	 ...
	 */
	public setRecords(languageListIndex: number, streamNumber: number, name: string, ...records: MetadataDescriptor[]): void {
		Guards.ushort(languageListIndex, "languageListIndex");
		Guards.ushort(streamNumber, "streamNumber");
