...
export default class HeaderObject extends BaseObject {
	...
	public get hasContentDescriptors(): boolean {
		return (
			this._children.findIndex((o) => {
				return o.objectType === ObjectType.ContentDescriptionObject || o.objectType === ObjectType.ExtendedContentDescriptionObject;
			}) >= 0
		);
	}
	...
	public get objectType(): ObjectType {
		return ObjectType.HeaderObject;
	}
	...
	public removeContentDescriptor(): void {
		for (let i = this._children.length - 1; i >= 0; i--) {
			if (this._children[i].objectType === ObjectType.ContentDescriptionObject || this._children[i].objectType === ObjectType.ExtendedContentDescriptionObject) {
	...
	public render(): ByteVector {
		...
		// Put it all together
		const output = ByteVector.concatenate(ReadWriteUtils.renderDWord(childCount), ReadWriteUtils.renderWord(this._reserved), childrenData);
