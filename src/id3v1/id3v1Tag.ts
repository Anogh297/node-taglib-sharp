...
export default class Id3v1Tag extends Tag {
	...
	public get performers(): string[] {
		return this._artist ? this._artist.split(";") : [];
	}
	/**
	 * @inheritDoc
	 * @remarks
	...
	public set performers(value: string[]) {
		this._artist = value ? value.join(";") : "";
	}

	/** @inheritDoc */
	public get album(): string {
		return this._album || undefined;
	}
	/**
	 * @inheritDoc
	 * @remarks
	...
	public set comment(value: string) {
		this._comment = value ? value.trim() : "";
	}
	...
	public set genres(value: string[]) {
		this._genre = !value || value.length === 0 ? 255 : Genres.audioToIndex(value[0].trim());
	}
	...
	private static renderField(value: string, maxLength: number): ByteVector[] {
		...
		return [ByteVector.fromString(valueToWrite, StringType.Latin1), remainingBytes > 0 ? ByteVector.fromSize(remainingBytes) : undefined];
	}
