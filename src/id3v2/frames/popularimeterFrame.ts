...
export default class PopularimeterFrame extends Frame {
	 ...
	 */
	public static fromOffsetRawData(data: ByteVector, offset: number, header: Id3v2FrameHeader, version: number): PopularimeterFrame {
	...
	public get frameClassType(): FrameClassType {
		return FrameClassType.PopularimeterFrame;
	}

	/**
	 * Gets the play count of the current instance
	 */
	public get playCount(): bigint {
	...
	protected renderFields(): ByteVector {
		...
		return ByteVector.concatenate(ByteVector.fromString(this._user, StringType.Latin1), ByteVector.getTextDelimiter(StringType.Latin1), this.rating, playCountData);
	}
}
