...
export default class AttachmentFrame extends Frame implements IPicture {
	...
	public static fromFile(file: IFileAbstraction, header: Id3v2FrameHeader, frameStart: number, size: number, version: number): AttachmentFrame {
	 ...
	 * @param version ID3v2 version the frame was originally encoded with
	 */
	public static fromOffsetRawData(data: ByteVector, offset: number, header: Id3v2FrameHeader, version: number): AttachmentFrame {
	...
	public get frameClassType(): FrameClassType {
		return FrameClassType.AttachmentFrame;
	}
	...
	public set type(value: PictureType) {
		...
		const frameId = value === PictureType.NotAPicture ? FrameIdentifiers.GEOB : FrameIdentifiers.APIC;
	 ...
	 */
	public static find(frames: AttachmentFrame[], description?: string, type: PictureType = PictureType.Other): AttachmentFrame {
		...
		return frames.find((f) => {
			...
			if (type !== PictureType.Other && f.type !== type) {
				return false;
			}
			return true;
		});
	...
	protected renderFields(version: number): ByteVector {
		...
		if (this.frameId === FrameIdentifiers.APIC) {
			...
			if (version === 2) {
				...
				extensionData = ByteVector.concatenate(ByteVector.fromString(this.mimeType, StringType.Latin1), ByteVector.getTextDelimiter(StringType.Latin1));
			}
	...
	private parseFromRawData(shouldRunFieldData: boolean): void {
		...
		const data = shouldRunFieldData ? super.fieldData(this._rawData, 0, this._rawVersion, false) : this._rawData;
		...
		if (this.frameId === FrameIdentifiers.APIC) {
			...
			this._filename = data.subarray(mimeTypeEndIndex + 1, filenameLength).toString(this._encoding);
			...
			this._description = data.subarray(filenameEndIndex + delim.length, descriptionLength).toString(this._encoding);
