...
export class EventTimeCode {
	...
	public render(): ByteVector {
		...
		return ByteVector.concatenate(this.eventType, ByteVector.fromInt(this.time));
	}
}
...
export class EventTimeCodeFrame extends Frame {
	 ...
	 */
	public static fromOffsetRawData(data: ByteVector, offset: number, header: Id3v2FrameHeader, version: number): EventTimeCodeFrame {
	...
	public get frameClassType(): FrameClassType {
		return FrameClassType.EventTimeCodeFrame;
	}
	 ...
	 */
	public get events(): EventTimeCode[] {
		return this._events || [];
	}
	/**
	 * Sets the event this frame contains
	 */
	public set events(value: EventTimeCode[]) {
		this._events = value;
	}
	 ...
	 */
	public set timestampFormat(value: TimestampFormat) {
	...
	protected parseFields(data: ByteVector): void {
		...
		for (let i = 1; i < data.length; i += 5) {
			...
			const timestampData = ByteVector.concatenate(data.get(i + 1), data.get(i + 2), data.get(i + 3), data.get(i + 4));
	...
	protected renderFields(): ByteVector {
		...
		const events = this.events.sort((a, b) => a.time - b.time).map((e) => e.render());

		return ByteVector.concatenate(this.timestampFormat, ...events);
	}
