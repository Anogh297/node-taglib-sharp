...
export default {
	...
	createFrame: (data: ByteVector, file: File, offset: number, version: number, alreadyUnsynced: boolean): { frame: Frame; offset: number } => {
		...
		try {
			...
			if (file) {
				...
				data = ByteVector.concatenate(data, file.readBlock(frameSize));
			}
