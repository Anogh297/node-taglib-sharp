...
export default class Opus implements IOggCodec, IAudioCodec {
	...
	public get description(): string {
		return `Opus v${this._opusVersion} Audio`;
	}
	...
	public writeCommentPacket(packets: ByteVector[], comment: XiphComment): void {
		...
		const data = ByteVector.concatenate(Opus.COMMENT_IDENTIFIER, comment.render(true));
	...
	public setDuration(firstGranularPosition: ByteVector, lastGranularPosition: ByteVector): void {
		...
		this._durationMilliseconds = durationMilliseconds > Number.MAX_SAFE_INTEGER ? -1 : Number(durationMilliseconds);
	}
