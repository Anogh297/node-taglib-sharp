...
export default class Vorbis implements IOggCodec, IAudioCodec {
	 ...
	 */
	public get commentData(): ByteVector {
		return this._commentData;
	}

	/** @inheritDoc */
	public get description(): string {
		return `Vorbis v${this._vorbisVersion} Audio`;
	}

	/** @inheritDoc */
	public get durationMilliseconds(): number {
		return this._durationMilliseconds || 0;
	}
	...
	public setDuration(firstGranularPosition: ByteVector, lastGranularPosition: ByteVector): void {
		...
		const durationMilliseconds = this._sampleRate === 0 ? NumberUtils.BIG_ZERO : ((lastLong - firstLong) * BigInt(1000)) / BigInt(this._sampleRate);

		// Note: if duration is > max safe, we cannot safely report the duration
		// @TODO: Make this configurable by the user
		this._durationMilliseconds = durationMilliseconds > Number.MAX_SAFE_INTEGER ? 0 : Number(durationMilliseconds);
	}
	...
	public writeCommentPacket(packets: ByteVector[], comment: XiphComment): void {
		...
		const data = ByteVector.concatenate(VorbisPacketType.Comment, Vorbis.IDENTIFIER, comment.render(true));
