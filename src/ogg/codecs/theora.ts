...
export default class Theora implements IOggCodec, IVideoCodec {
	...
	public get description(): string {
		return `Theora v${this._majorVersion}.${this._minorVersion} Video`;
	}
	...
	public setDuration(firstGranularPosition: ByteVector, lastGranularPosition: ByteVector): void {
		...
		this._durationMilliseconds = durationMilli > Number.MAX_SAFE_INTEGER ? 0 : Number(durationMilli);
	}
	...
	public writeCommentPacket(packets: ByteVector[], comment: XiphComment): void {
		...
		const data = ByteVector.concatenate(TheoraPacketType.CommentHeader, Theora.IDENTIFIER, comment.render(true));
