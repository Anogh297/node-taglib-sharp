	...
	public paginate(): { change: number; pages: OggPage[] } {
		...
		while (packets.length > 0) {
			...
			if (lacingBytesUsed === lacingPerPage) {
				const header = OggPageHeader.fromPageHeader(firstHeader, index, firstPacketContinued ? OggPageFlags.FirstPacketContinued : OggPageFlags.None);
		...
		if (pagePackets.length > 0) {
			const header = OggPageHeader.fromInfo(firstHeader.streamSerialNumber, index, firstPacketContinued ? OggPageFlags.FirstPacketContinued : OggPageFlags.None);
			pages.push(OggPage.fromPackets(header, pagePackets));
			index++;
			count--;
		}
