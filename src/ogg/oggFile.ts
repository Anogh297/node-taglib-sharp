	...
	public constructor(file: IFileAbstraction | string, readStyle: ReadStyle) {
		...
		try {
			...
			for (const [id, bitStream] of streamsResult.streams.entries()) {
				commentMapping.set(id, XiphComment.fromData(bitStream.codec.commentData, NumberUtils.hasFlag(readStyle, ReadStyle.PictureLazy)));
			...
			if (NumberUtils.hasFlag(readStyle, ReadStyle.Average)) {
				...
				streamsResult.streams.get(lastPageHeader.streamSerialNumber).setDuration(lastPageHeader.absoluteGranularPosition);
				const duration = streamsResult.streams.get(lastPageHeader.streamSerialNumber).codec.durationMilliseconds;
		...
		try {
			...
			for (const page of pages) {
				...
				if (!paginators.has(streamSerialNumber)) {
					paginators.set(streamSerialNumber, new OggPaginator(streamsResult.streams.get(streamSerialNumber).codec));
				}
