...
export default class OggPage {
	 ...
	 */
	public get header(): OggPageHeader {
		return this._header;
	}
	...
	public get size(): number {
		return this._header.size + this._header.dataSize;
	}
	...
	public static overwriteSequenceNumbers(file: File, position: number, shiftTable: Map<number, number>): void {
		...
		while (position < file.length - OggPageHeader.MINIMUM_SIZE) {
			...
			if (shiftTable.has(header.streamSerialNumber) && shiftTable.get(header.streamSerialNumber) !== 0) {
				...
				const newData = ByteVector.fromUint(header.pageSequenceNumber + shiftTable.get(header.streamSerialNumber), false);
	...
	public render(): ByteVector {
		const data = ByteVector.concatenate(this._header.render(), ...this._packets);
