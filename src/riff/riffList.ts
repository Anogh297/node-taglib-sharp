...
export default class RiffList implements IRiffChunk, ILazy {
	...
	/** @inheritDoc */
	public get chunkStart(): number | undefined {
		return this._chunkStart;
	}
	/** @inheritDoc */
	...
	/** @inheritDoc */
	public get fourcc(): string {
		return RiffList.IDENTIFIER_FOURCC;
	}
	...
	/** @inheritDoc */
	public get originalDataSize(): number {
		return this._originalDataSize;
	}
	...
	public render(): ByteVector {
		...
		const valueData = Array.from(this._values.entries(), ([key, value]) => {
			const valuesBytes = value.map((v) =>
				ByteVector.concatenate(ByteVector.fromString(key, StringType.UTF8), ByteVector.fromUint(v.length, false), v, v.length % 2 === 1 ? 0x00 : undefined),
			);
		...
		const allData = ByteVector.concatenate(...valueData, ...listData);
