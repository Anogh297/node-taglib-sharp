		...
		try {
			...
			if (taggingChunkIndexes.length > 0) {
				...
				if (taggingChunksAreContiguous) {
					...
					taggingChunkLength = this._rawChunks[lastChunkIndex].chunkStart + this._rawChunks[lastChunkIndex].originalTotalSize - taggingChunkStart;
					...
					for (const chunkIndex of taggingChunkIndexes) {
						...
						const blockLength = this._rawChunks[lastChunkIndex].chunkStart + this._rawChunks[lastChunkIndex].originalTotalSize - blockStart;
	...
	private read(readStyle: ReadStyle): void {
		...
		try {
			...
			while (position + 8 < this.length) {
				...
				const chunk = fourcc === RiffList.IDENTIFIER_FOURCC ? RiffList.fromFile(this, position) : RiffChunk.fromFile(this, fourcc, position);
			...
			const divxTag = divxChunkIndex >= 0 ? DivxTag.fromData((<RiffChunk>this._rawChunks[divxChunkIndex]).data) : undefined;
			...
			const id3v2Tag = id3v2ChunkIndex >= 0 ? Id3v2Tag.fromData((<RiffChunk>this._rawChunks[id3v2ChunkIndex]).data) : undefined;

			// 3) Info tag
			const infoTagChunkIndex = this._taggingChunkIndexes.get(TagTypes.RiffInfo);
			const infoTag = infoTagChunkIndex >= 0 ? InfoTag.fromList(<RiffList>this._rawChunks[infoTagChunkIndex]) : undefined;

			// 4) MovieID tag
			const movieIdChunkIndex = this._taggingChunkIndexes.get(TagTypes.MovieId);
			const moveIdTag = movieIdChunkIndex >= 0 ? MovieIdTag.fromList(<RiffList>this._rawChunks[movieIdChunkIndex]) : undefined;
		...
		switch (this._fileType) {
			...
			case "AVI ":
				...
				const aviHeaderList = this._rawChunks.find((c) => {
					return c.fourcc === RiffList.IDENTIFIER_FOURCC && (<RiffList>c).type === AviHeader.HEADER_LIST_TYPE;
				});
				if (!aviHeaderList) {
