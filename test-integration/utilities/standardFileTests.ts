...
export class StandardFileTests {
	...
	public static writeStandardPictures(sampleFile: string, tmpFile: string, readStyle: ReadStyle = ReadStyle.Average, level: TestTagLevel = TestTagLevel.Medium) {
		if (fs.existsSync(tmpFile)) {
			fs.unlinkSync(tmpFile);
		}
	...
	public static writeStandardTags(sampleFile: string, tmpFile: string, level: TestTagLevel = TestTagLevel.Normal, types: TagTypes = TagTypes.AllTags) {
		if (sampleFile !== tmpFile && fs.existsSync(tmpFile)) {
			fs.unlinkSync(tmpFile);
		}
